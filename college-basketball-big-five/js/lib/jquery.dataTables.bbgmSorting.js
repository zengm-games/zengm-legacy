...
//                if (Char === "M") {
    ...
    $.fn.dataTableExt.oSort["money-asc"] = function (a, b) {
		...
		if (a.length<6) {
		// if less than 1k, no comma
			x = parseFloat(a.substring(a.indexOf("$") + 1, a.length));
		} else {
		// if 1k or greater, has comma		
			x = parseFloat(a.substring(a.indexOf("$") + 1, a.indexOf(",")));
			x *= 1000;
			z = parseFloat(a.substring(a.indexOf(",") + 1, a.length));
			x += z;
		}
		...
		if (b.length<6) {
			y = parseFloat(b.substring(b.indexOf("$") + 1, b.length));
		} else {
			y = parseFloat(b.substring(b.indexOf("$") + 1, b.indexOf(",")));
			y *= 1000;
			zz = parseFloat(b.substring(b.indexOf(",") + 1, b.length));
			y += zz;
		}		

    ...
    function updateRoster(inputs, updateEvents, vm) {
        ...
        if (updateEvents.indexOf("dbChange") >= 0 || (inputs.season === g.season && (updateEvents.indexOf("gameSim") >= 0 || updateEvents.indexOf("playerMovement") >= 0)) || inputs.abbrev !== vm.abbrev() || inputs.season !== vm.season()) {
            ...
            return team.filter({
                ...
                seasonAttrs: ["profit", "cash","won", "lost", "playoffRoundsWon", "playoff64RoundsWon","hype"],
                ot: tx
            }).then(function (t) {
                var attrs, ratings, stats,i;

                vars.team = t;

				for (i = 0; i < t.length; i++) {
				 vars.team[i].hype *= 100;
				 vars.team[i].hype = Math.round(vars.team[i].hype);
				}

				
                attrs = ["pid", "tid", "draft", "name", "pos", "age","year", "contract", "cashOwed", "rosterOrder", "injury", "ptModifier", "watch","city","state","miles"];  // tid and draft are used for checking if a player can be released without paying his salary

...
//                    baseMoods[i] += (teams[i].seasons[s].playoff64RoundsWon)*.2 (teams[i].seasons[s].playoffRoundsWon)*.1;
    ...
    function addToFreeAgents(ot, p, phase, baseMoods) {
		...
		//console.log("before: :"+p.tid);
        p.tidBackup = p.tid;
        p.tid = g.PLAYER.FREE_AGENT;
		//console.log("afer: :"+p.tid+" backup: :"+p.tidBackup);
...
//            va = mins[i] * (pers[i] - prls[p.pos]*3) / 67;
//            ewas.push(va / 30 * 0.8); // 0.8 is a fudge factor to approximate the difference between (in-game) EWA and (real) win shares
            if ( (mins[i] < 10) && (mins.length == 1) ) {
				...
				va = mins[i] * (pers[i] - prls[p.pos]*2.5*1)/30 ;
			}
...
//            return 0.4 * potential + 0.6 * current;
    ...
    function retire(tx, p, playerStats) {		
       ...
       // p.lastTeam = p.teamRegion;
		//console.log(p.teamRegion);
        // Add to Hall of Fame?

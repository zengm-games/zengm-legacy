...
//						playerSigned = 	Math.round(players.length*(.03));
						playerSigned = 	10;
						if (g.daysLeft == 0) {
							playerSigned = 	players.length;		// fix recruiting skip bug?				
							/*if (playerSigned >= 600) {
								g.daysLeft = 30;
							}*/
						}
...
//							         amount *= 1 + (miles/300-1)*.5;
						//	console.log("tid: "+tid+" hype: "+teams[tid].hype);
//							console.log("tid: "+tid+" hype: "+teams[tid].seasons[t.seasons.length - 1].hype);
						//	console.log("tid: "+tid+" hype: "+hype[tid]);
							
							//t.seasons[t.seasons.length - 1].cash
							

							if (hype[tid]>.85) {
								contractMaxAmount = (players[i].contract.amount)*(1+(players[i].miles[tid]/1200-1)*.5)/ ((hype[tid]+1)/3)  ;
...
//						    if ( ( (  (contractAmount + payroll) <= (cash[tid]/(13-numPlayersOnRoster))) && (numPlayersOnRoster < 13) ) || ( (  (contractAmount ) <= 0) && (numPlayersOnRoster < 13) )  ) {
						...
						//	console.log("tid: "+tid+" cash: "+cash[tid]+" payroll: "+payroll+" contractAmount: "+contractAmount+" maxContract: "+maxContract+" numPlayersOnRoster:"+numPlayersOnRoster);
                        ...
                        require("core/league").setGameAttributes({daysLeft: g.daysLeft - 1, lastDbChange: Date.now()}).then(function () {
                            if (g.daysLeft > 0 && numDays > 0) {
                                ui.realtimeUpdate(["playerMovement"], undefined, function () {
									console.log("days Left: "+g.daysLeft)								

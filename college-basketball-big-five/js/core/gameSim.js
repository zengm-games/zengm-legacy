...
//                this.t = 10;
    ...
    GameSim.prototype.updatePlayersOnCourt = function () {
        ...
        for (t = 0; t < 2; t++) {
			...
			//	    console.log("injured: "+this.team[t].player[p].injured );
			//	    console.log("fouled out: "+this.team[t].player[p].stat.pf);					
                    ovrs[p] = -Infinity;
                } else {
    ...
    GameSim.prototype.ratingArray = function (rating, t, power) {
        ...
        for (i = 0; i < 5; i++) {
            p = this.playersOnCourt[t][i];
            array[i] = Math.pow(this.team[t].player[p].compositeRating[rating] * this.fatigue(this.team[t].player[p].stat.energy / 10), power);
        }

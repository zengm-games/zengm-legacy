...
define(["dao", "db", "globals", "ui", "core/freeAgents", "core/finances", "core/gameSim", "core/league", "core/player", "core/season", "core/team", "lib/bluebird", "util/advStats", "util/eventLog", "util/lock", "util/helpers", "util/random"], function (dao, db, g, ui, freeAgents, finances, gameSim, league, player, season, team, Promise, advStats, eventLog, lock, helpers, random) {
    ...
    function writeTeamStats(tx, results) {
		...
		/*	console.log("t1: "+t1);
			console.log("id: "+results.team[t1].id);
			console.log("t1: "+t2);
			console.log("id: "+results.team[t2].id);*/
			////////////////////// try to save this?
     /*      return Promise.all([
                team.filter({
		...
		//		console.log("game pts: "+game.length);
			/*	console.log("tid: "+team[t1].tid);
				console.log("cid: "+team[t1].cid);
				console.log("did: "+team[t1].did);*/
...
//				merchRevenue = 100;
		...
		//			merchRevenue += 50;
                ...
                if ((g.phase === g.PHASE.PLAYOFFS) ) {
                    if (won === true) {
			//			localTvRevenue += 2000;
					//	teamSeason.hype = teamSeason.hype*.98 + 0.02;
					} else {
					//	teamSeason.hype = teamSeason.hype*.95 - 0.05;
					}
                }
				// national wins
                if ((g.phase === g.PHASE.PLAYOFFS64) ) {
                    if (won === true) {
						//teamSeason.hype = teamSeason.hype*.97 + 0.03;
...
//                    teamSeason.hype = teamSeason.hype*.985 + 0.005 * (winp - 0.55) + 0.0075 * (winp - winpOld);
//                    teamSeason.hype += 0.005 * (winp - 0.55) + 0.0075 * (winp - winpOld);
//                    teamSeason.hype += 0.003 * (winp - winpOld);
                    teamSeason.hype *= 0.997;
                    teamSeason.hype += 0.003*winp;
//                    teamSeason.hype += 0.003 * (winp - winpOld);
//                    teamSeason.hype = teamSeason.hype*.9975 + 0.0025*(win+winp*winp)/2;
//                    teamSeason.hype = teamSeason.hype*.9975 + 0.0025*(winp*winp*winp);
                    if (teamSeason.hype > 1) {
                        teamSeason.hype = 1;
                    } else if (teamSeason.hype < 0) {
                        teamSeason.hype = 0;
...
//                revenue = merchRevenue + sponsorRevenue + nationalTvRevenue + localTvRevenue + ticketRevenue+5000;
...
//                teamSeason.revenues.nationalTv.amount += 10000;
...
//                } else if (g.phase !== g.PHASE.PLAYOFFS && g.phase !== g.PHASE.PLAYOFFS64) {
    ...
    function writePlayerStats(tx, results) {
        ...
        //         console.log("got here2?");
					...
					//	console.log(p.injured);
					//	console.log(injuredThisGame);
					//	console.log(p.injury.type);
				...
				return dao.playoffSeries64.get({ot: tx, key: g.season}).then(function (playoffSeries64) {
					...
					if ((g.userTid === results.team[0].id || g.userTid === results.team[1].id) && playoffSeries64.currentRound < 5) {
						if (series.away.won === 1 || series.home.won === 1) {
							otherTid = g.userTid === results.team[0].id ? results.team[1].id : results.team[0].id;
							loserWon = series.away.won === 1 ? series.home.won : series.away.won;
							if (playoffSeries64.currentRound === 0) {
					//			currentRoundText = "round of 64";
					//		} else if (playoffSeries64.currentRound === 1) {
								currentRoundText = "round of 32";
							} else if (playoffSeries64.currentRound === 1) {
								currentRoundText = "sweet 16";
							} else if (playoffSeries64.currentRound === 2) {
								currentRoundText = "elite 8";
							} else if (playoffSeries64.currentRound === 3) {
								currentRoundText = "final 4";
							} else if (playoffSeries64.currentRound === 4) {
...
//									text: 'Your team was eliminated by the <a href="' + helpers.leagueUrl(["roster", g.teamAbbrevsCache[otherTid], g.season]) + '">' + g.teamNamesCache[otherTid] + '</a> in the ' + currentRoundText + '.'
					...
					// If somebody just won the title, announce it
					if (playoffSeries64.currentRound === 4 && (series.away.won === 1 || series.home.won === 1)) {
						if ((series.away.tid === g.userTid && series.away.won === 1) || (series.home.tid === g.userTid && series.home.won === 1)) {
							eventLog.add(tx, {
								type: "playoffs64",
    ...
    function play(numDays, start, gidPlayByPlay) {
        ...
        cbNoGames = function () {
            ...
            return league.setGameAttributes({gamesInProgress: false}).then(function () {
                ...
                if ((g.phase < g.PHASE.PLAYOFFS64 ) && (g.phase >= g.PHASE.PLAYOFFS )) {
        ...
        cbSaveResults = function (results) {
            ...
            tx = dao.tx(["events", "games", "players", "playerStats", "playoffSeries","playoffSeries64", "releasedPlayers", "schedule", "teams"], "readwrite");
...
//console.log(results[i]);
			...
			//                            if (g.phase === g.PHASE.PLAYOFFS) {
							...
							//				if ( (g.phase === g.PHASE.PLAYOFFS)  ) {
								...
								//			} else  (  (g.phase === g.PHASE.PLAYOFFS64) ) {
											
									//			season.newSchedulePlayoffsDay().then(function () {
			//									season.newSchedulePlayoffsDay64().then(function () {
										//			play(numDays - 1, false);
											//    });									
										   // }
...
//                            if ((g.phase !== g.PHASE.PLAYOFFS) ) {
						//		console.log("each day");
                            ...
                            if ((g.phase === g.PHASE.FREE_AGENCY) ) {
								...
								//// don't allow resigning during season
					//			console.log("each day: free agency");

    ...
    function updateOwnerMood() {
        return team.filter({
            ...
            if (t.playoff64RoundsWon < 0) {
                deltas.playoffs = -.2;
            } else if (t.playoff64RoundsWon < 5) {
//            } else if (t.playoffRoundsWon < 4) {
                deltas.playoffs = 0.025 * t.playoff64RoundsWon;
            } else {
                deltas.playoffs = 0.2;
            }
...
*/
            ...
            //awardsByPlayer.push({pid: p.pid, tid: p.tid, name: p.name, type: "CT MVP"});
...
*/
            ...
            players.sort(function (a, b) {  return b.statsPlayoffs.ewa - a.statsPlayoffs.ewa; });
            p = players[0];
			console.log(p.abbrev+" "+p.name);
            awards.finalsMvp = {pid: p.pid, name: p.name, tid: p.tid, abbrev: p.abbrev, pts: p.statsPlayoffs.pts, trb: p.statsPlayoffs.trb, ast: p.statsPlayoffs.ast};
            awardsByPlayer.push({pid: p.pid, tid: p.tid, name: p.name, type: "NT MVP"});
...
//				    console.log("phaseText[phase]");		
    ...
    function newPhasePreseason() {
        ...
        return freeAgents.autoSign().then(function () { // Important: do this before changing the season or contracts and stats are fucked up
			...
			//// check team size while recruiting is still available
...
//			});			*/
    ...
    function newPhaseRegularSeason() {
		...
		//console.log("to regular seasons")
            ...
            dao.teams.iterate({
				...
				//	console.log("t.did: "+t.did);
                    if (tidPlayoffs.indexOf(t.tid) >= 0) {
						...
						// lower all hype at start, better teams will get some back, but top teams need to make NT
...
//            }).then(updateOwnerMood).then(function () {
      ...
      //          ui.updateStatus("Idle");
			...
			//	console.log(url);
//                return newPhaseFinalize(g.PHASE.BEFORE_PLAYOFFS64, url, ["playerMovement"]);
                return newPhaseFinalize(g.PHASE.BEFORE_PLAYOFFS64, url, ["teamFinances"]);
//                return newPhaseFinalize();
//                return newPhaseFinalize(g.PHASE.PLAYOFFS, url, ["teamFinances"]);
//                return newPhaseFinalize(g.PHASE.BEFORE_PLAYOFFS64);
            }).then(function () {
...
/*					teamSOS[i] = (teamOppWins[i]/(teamOppLosses[i]+teamOppWins[i])*2+teamOppOppWins[i]/(teamOppOppLosses[i]+teamOppOppWins[i]))/3;
			...
			//	var tempParse;
				...
				//	tempParse = (parseInt(teams[i].lastTen.split("-")[0], 10));
				//	console.log(teams[i].diff+" "+tempParse);
                }						  
...
//				console.log("teams[i].revenues.nationalTv.amount: "+teams[i].revenues.nationalTv.amount);	
	//			console.log("playoff success: "+playoffSuccess[teams[i].cid]);
				teams[i].revenues.nationalTv.amount += playoffSuccess[teams[i].cid]*1000;
	//			console.log("revenue: "+teams[i].revenues.nationalTv.amount);							
//				console.log("teams[i].revenues.nationalTv.amount: "+teams[i].revenues.nationalTv.amount);		
...
//            dao.players.iterate({
            dao.teams.iterate({
                ...
                callback: function (t) {			
						var tid;
						tid = t.tid;
					//	console.log("revenue: "+teams[tid].revenues.nationalTv.amount);
...
//						console.log("yearsWithTeam: "+YWT);
//						console.log("yearsWithTeam: "+YWT.length);
//						console.log("yearsWithTeam: "+YWT[YWT.length-1]);
//						console.log("yearsWithTeam: "+YWT[YWT.length-2]);

					//	console.log("yearsWithTeam: "+p.stats[p.stats.length - 1].yearsWithTeam);
					//	console.log("season: "+g.season);
...
//                            if (age > 22 || p.tid === g.PLAYER.FREE_AGENT) {  // Only players older than 34 or without a contract will retire
								...
								//	console.log("playersLeft[i]: "+playersLeft[p.tid]);
									...
									dao.teams.iterate({
										...
										callback: function (t) {			
												...
												//	console.log("t.tid: "+t.tid);
												if (t.tid == 1) {
													console.log("p.tid: "+p.tid+" pTid: "+pTid+" t.tid: "+t.tid);
												}
												if (pTid == t.tid) {
													...
													t.seasons[t.seasons.length - 1].revenues.merch.amount += 15000;
													t.seasons[t.seasons.length - 1].cash += 15000;
													console.log("t.seasons[t.seasons.length - 1].cash: "+t.seasons[t.seasons.length - 1].cash);
  ...
  //                      }
                        ...
                        if (p.tid === g.PLAYER.FREE_AGENT) {
                           // if (p.yearsFreeAgent >= 1) {
							    // don't want free agents to retire, just graduate
                                //p = player.retire(tx, p, playerStats);
...
/*		team.filter({
		...
		//				ot: ot,					
		    ...
		    console.log(teams[g.userTid].tid);
        });*/
...
//                    key: g.PLAYER.UNDRAFTED, // This only works because g.PLAYER.UNDRAFTED is -2 and g.PLAYER.FREE_AGENT is -1
                    callback: function (p) {
           ...
           /*     }).then(function () {
			...
			//						return draft.genPlayers(null, g.PLAYER.UNDRAFTED_3).then(function() {
									return draft.genPlayers(null, g.PLAYER.FREE_AGENT,null,null,t).then(function() {
										console.log("new free agents")
								...
								console.log("no new free agents")
							}
						}						
                    });				
*/
              ...
              /*  }).then(function () {
					...
					// store the first time and do again in case first time doesn't work?
		...
		//                    key: g.PLAYER.UNDRAFTED, // This only works because g.PLAYER.UNDRAFTED is -2 and g.PLAYER.FREE_AGENT is -1
							...
							return dao.players.iterate({
								ot: tx,
								index: "tid",
								key: g.PHASE.FREE_AGENCY,
								callback: function (p) {
									var contract, factor;

									//// shouldn't be needed
									if (p.contract.exp <= g.season && p.tid !== g.userTid && p.tidBackup == -1) {
            ...
            if (rnd === 4) {
                ...
                dao.teams.iterate({
                    ...
                    callback: function (t) {
                        ...
                        t.seasons[s].playoff64RoundsWon = 5;

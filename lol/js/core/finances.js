...
define(["dao", "globals", "lib/underscore"], function (dao, g, _) {
    ...
    function getRankLastThree(t, category, item) {
        ...
        if (s > 0) {
            ...
            return (t.seasons[s][category][item].rank + t.seasons[s - 1][category][item].rank + (g.numTeams+1)/2) / 3;
        }
        return (t.seasons[s][category][item].rank + (g.numTeams+1)/2 + (g.numTeams+1)/2) / 3;
    }

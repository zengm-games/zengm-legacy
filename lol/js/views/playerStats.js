        ...
        if (updateEvents.indexOf("dbChange") >= 0 || (inputs.season === g.season && (updateEvents.indexOf("gameSim") >= 0 || updateEvents.indexOf("playerMovement") >= 0)) || inputs.abbrev !== vm.abbrev() || inputs.season !== vm.season() || inputs.statType !== vm.statType() || inputs.playoffs !== vm.playoffs()) {
            return dao.players.getAll({
                ...
                players = player.filter(players, {
                    attrs: ["pid", "name", "pos", "age", "injury", "tid", "hof", "watch","born"],
                    ratings: ["skills"],
                    stats: ["abbrev", "tid", "gp", "gs", "min", "fg", "fga", "fgp", "tp", "tpa", "tpp", "ft", "fta", "ftp", "orb", "drb", "trb", "ast", "tov", "stl", "blk", "pf", "pts", "per", "ewa","fgLowPost","fgaLowPost","fgMidRange","oppJM","kda","scTwr","scKills"],
					
//                stats: ["min","gp", "fg", "fga", "fgp", "tp", "tpa", "tpp", "ft", "fta", "ftp", "orb", "drb", "trb", "ast", "tov", "stl", "blk", "pf", "pts", "oppPts", "diff","fgLowPost","fgaLowPost","fgMidRange","oppJM"],
                ...
                for (i = 0; i < players.length; i++) {
                    if (players[i].stats.min > gp) {
                        gp = players[i].stats.min;
...
//                    rows.push([helpers.playerNameLabels(p.pid, p.name, p.injury, p.ratings.skills, p.watch), p.pos, '<a href="' + helpers.leagueUrl(["roster", abbrev, season]) + '">' + abbrev + '</a>', String(p.stats.gp), String(p.stats.gs), helpers.round(p.stats.min, d), helpers.round(p.stats.fg, d), helpers.round(p.stats.fga, d), helpers.round(p.stats.fgp, 1), helpers.round(p.stats.tp, d), helpers.round(p.stats.tpa, d), helpers.round(p.stats.tpp, 1), helpers.round(p.stats.ft, d), helpers.round(p.stats.fta, d), helpers.round(p.stats.ftp, 1), helpers.round(p.stats.orb, d), helpers.round(p.stats.drb, d), helpers.round(p.stats.trb, d), helpers.round(p.stats.ast, d), helpers.round(p.stats.tov, d), helpers.round(p.stats.stl, d), helpers.round(p.stats.blk, d), helpers.round(p.stats.pf, d), helpers.round(p.stats.blk, d), helpers.round(p.stats.blk, 1), helpers.round(p.stats.blk, 1), p.hof, tid === g.userTid]);
                    rows.push([helpers.playerNameLabels(p.pid, p.name, p.injury, p.ratings.skills, p.watch), p.pos, '<a href="' + helpers.leagueUrl(["roster", abbrev, season]) + '">' + abbrev + '</a>',p.born.loc, String(p.stats.gp), helpers.round(p.stats.min, d), helpers.round(p.stats.fg, 1), helpers.round(p.stats.fga, 1), helpers.round(p.stats.fgp, 1), helpers.round(p.stats.kda, 1), helpers.round(p.stats.scKills, 1), helpers.round(p.stats.pf, 1), helpers.round(p.stats.orb, 1), helpers.round(p.stats.scTwr, 1), helpers.round(p.stats.fgaLowPost, 1), helpers.round(p.stats.fgLowPost, 1), helpers.round(p.stats.tp, 1), helpers.round(p.stats.tpa, 1), helpers.round(p.stats.ft, 1), helpers.round(p.stats.fta, 1), helpers.round(p.stats.fgMidRange, 1), helpers.round(p.stats.oppJM, 1), helpers.round(p.stats.drb, 1), helpers.round(p.stats.blk, 1), helpers.round(p.stats.tov, 1), helpers.round(p.stats.ast, 1), helpers.round(p.stats.trb, 1), p.hof, tid === g.userTid]);
                }
            }

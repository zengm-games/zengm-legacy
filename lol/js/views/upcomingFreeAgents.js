...
//                players[i].contractDesired.amount /= 1000;
                players[i].contractDesired.amount /= 1;
                players[i].contractDesired.exp += inputs.season - g.season;
            }

            players = player.filter(players, {
                attrs: ["pid", "name", "pos", "age", "contract", "freeAgentMood", "injury", "watch", "contractDesired","region","born"],
...
//                return [helpers.playerNameLabels(p.pid, p.name, p.injury, p.ratings.skills, p.watch), p.pos, String(p.age), String(p.ratings.ovr), String(p.ratings.pot), helpers.round(p.stats.min, 1), helpers.round(p.stats.pts, 1), helpers.round(p.stats.trb, 1), helpers.round(p.stats.ast, 1), helpers.round(p.stats.per, 1), helpers.formatCurrency(p.contract.amount, "M") + ' thru ' + p.contract.exp, helpers.formatCurrency(p.contractDesired.amount, "M") + ' thru ' + p.contractDesired.exp];
                return [helpers.playerNameLabels(p.pid, p.name, p.injury, p.ratings.skills, p.watch), p.pos, String(p.age), String(p.born.loc), String(p.ratings.ovr), String(p.ratings.pot), helpers.round(p.stats.min, 1), helpers.round(p.stats.fg, 1), helpers.round(p.stats.fga, 1), helpers.round(p.stats.fgp, 1), helpers.round(p.stats.tp, 1), helpers.formatCurrency(p.contract.amount, "K") + ' thru ' + p.contract.exp, helpers.formatCurrency(p.contractDesired.amount, "K") + ' thru ' + p.contractDesired.exp];
//                return [helpers.playerNameLabels(p.pid, p.name, p.injury, null, p.watch), p.pos, String(p.age), String(p.born.loc), String(p.ratings.ovr), String(p.ratings.pot), helpers.round(p.stats.min, 1), helpers.round(p.stats.fg, 1), helpers.round(p.stats.fga, 1), helpers.round(p.stats.fgp, 1), helpers.round(p.stats.tp, 1), helpers.formatCurrency(p.contract.amount, "K") + ' thru ' + p.contract.exp, helpers.formatCurrency(p.contractDesired.amount, "K") + ' thru ' + p.contractDesired.exp];

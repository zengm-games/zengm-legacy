...
define(["dao", "globals", "ui", "core/finances", "core/team", "lib/jquery", "lib/knockout", "lib/underscore", "views/components", "util/bbgmView", "util/helpers"], function (dao, g, ui, finances, team, $, ko, _, components, bbgmView, helpers) {
    ...
    function enableFinanceSettings(tid) {
        $("#finances-settings button").html("Save Expense Settings");
        if (tid === g.userTid) {
...
//                        budget[key] = helpers.round(budget[key] * 1) * 10 /1000;
                        budget[key] = helpers.round(budget[key] * 1) * 1 ;
...
//        this.payroll *= 1000;
            ...
            return team.getPayroll(null, inputs.tid).get(1).then(function (contracts) {
                ...
                for (i = 0; i < contracts.length; i++) {
                    ...
                    for (j = season; j <= contracts[i].exp; j++) {
                        ...
                        contracts[i].amounts.push(contracts[i].amount / 1);
                        contractTotals[j - season] += contracts[i].amount / 1;
                    }
                ...
                return team.filter({
                    ...
                    vars.team = t;
				//	console.log(t.payroll);
                    vars.payroll = t.payroll*1000; // For above/below observables
					
					//console.log(vars.payroll);
                    return vars;
                });
            });
        }
    ...
    function uiFirst(vm) {
        ko.computed(function () {
            ui.title(vm.team.region()  + " Finances");
        }).extend({throttle: 1});
        ...
        $("#help-expense-settings").popover({
            ...
            html: true,
            content: "<p>Analysts: Controls the accuracy of displayed player ratings.<p></p>Coaching: Better coaches mean better player development.</p></p>Gaming House: Better training facilities make your players happier and other players envious."
        });
        ...
        ko.computed(function () {
			...
			//	p.name = p.name.split(" ")[0] + " '"+p.userID +"' " + p.name.split(" ")[1];
                ...
                for (i = 0; i < 5; i++) {
                    if (p.amounts[i]) {
                        output.push(helpers.formatCurrency(p.amounts[i], "K"));
                    } else {
                        output.push("");
                    }
                    if (p.released) {
                ...
                footerCallback: function (tfoot) {
                    ...
                    for (i = 0; i < contractTotals.length; i++) {
                        cells[i + 1].innerHTML = helpers.formatCurrency(contractTotals[i], "k");
                    }
                }
            });
        }).extend({throttle: 1});
		...
		//	console.log(barData.expenses.salary);			
		//	barData.expenses.salary*= 1000;
		//	console.log(barData.expenses.salary);
		//	barData.expenses.salary[0] *= 1000;
		//	console.log(barData.expenses.salary[0]);
		//	console.log(barData.expenses[0]);
		
			if (g.gameType == 0) {
				$.barGraph($("#bar-graph-won"), barData.won, [0, 18], barSeasons);
			} else if (g.gameType == 1) {
				$.barGraph($("#bar-graph-won"), barData.won, [0, 26], barSeasons);
			} else if (g.gameType == 2) {
				$.barGraph($("#bar-graph-won"), barData.won, [0, 18], barSeasons);
			} else if (g.gameType == 3) {
				$.barGraph($("#bar-graph-won"), barData.won, [0, 22], barSeasons);
			} else if (g.gameType == 4) {
				$.barGraph($("#bar-graph-won"), barData.won, [0, 14], barSeasons);
			} else {
				$.barGraph($("#bar-graph-won"), barData.won, [0, 22], barSeasons);
			}
            ...
            $.barGraph($("#bar-graph-pop"), barData.pop, [0, 20], barSeasons, function (val) {
                return helpers.round(val, 1) + "K";
            });
            $.barGraph($("#bar-graph-att"), barData.att, [0, 75000], barSeasons, function (val) {
            ...
            $.barGraph(
                ...
                [
                    barSeasons,
                    ["tournament winnings", "local TV revenue", "ticket revenue", "corporate sponsorship revenue", "merchandising revenue"]
                ],
                function (val) {
//                    return helpers.formatCurrency(val / 1000, "K", 1);
					
//                    return helpers.formatCurrency(val /1000, "", 1);
                    return helpers.formatCurrency(val/1000, "K", 1);
                }
            );
            $.barGraph(
...
//                    return helpers.formatCurrency(val / 1000, "K", 1);
            ...
            $.barGraph($("#bar-graph-cash"), barData.cash, undefined, barSeasons, function (val) {
                return helpers.formatCurrency(val, "K", 1);
            });
        }).extend({throttle: 1});
    }

...
define(["dao", "globals", "lib/bluebird", "lib/davis", "lib/underscore", "util/eventLog", "util/helpers"], function (dao, g, Promise, Davis, _, eventLog, helpers) {
    ...
    function abortHandler(event) {
        ...
        if (event.target.error.name === "QuotaExceededError") {
            eventLog.add(null, {
                type: "error",
                text: 'Your browser isn\'t letting LOL GM store any more data!<br><br>Try <a href="/">deleting some old leagues</a> or deleting old data (Tools > Improve Performance within a league). Clearing space elsewhere on your hard drive might help too.',
                saveToDb: false
            });
        } else {
        ...
        require("core/league").loadGameAttributes(tx).then(function () {
            ...
            if (event.oldVersion <= 13) {
                (function () {
                    ...
                    if (g.regionalRestrictions === undefined) {
                        tx.objectStore("gameAttributes").put({
                            key: "regionalRestrictions",
                            value: true
                        });
                    }							

...
console.log(g.vm.multiTeam);
    ...
    function title(text) {
		...
		// change for each game type
		if (g.gameType == 0) {
			document.title = text + " - LOL GM LCS";
		} else if (g.gameType == 1) {
			document.title = text + " - LOL GM LCS w/ Ladder";
		} else if (g.gameType == 2) {
			document.title = text + " - LOL GM LCK";
		} else if (g.gameType == 3) {
			document.title = text + " - LOL GM LPL";
		} else if (g.gameType == 4) {
			document.title = text + " - LOL GM LMS";
		} else if (g.gameType == 5) {
			document.title = text + " - LOL GM Worlds";
		} else {
			document.title = text + " - LOL GM";
		}
    }
    ...
    function updatePlayMenu(ot) {
		...
		if (g.phase === g.PHASE.REGULAR_SEASON) {
			allOptions = [{id: "play-menu-stop", url: "", label: "Stop"},
						  ...
						  {id: "play-menu-month", url: "", label: "Until playoffs"},

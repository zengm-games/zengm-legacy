...
define(["globals", "ui", "core/league", "util/bbgmView"], function (g, ui, league, bbgmView) {
    ...
    function genFileName(data) {
        var fileName, i, leagueName, playoffSeries,playoffSeries64, rnd, season, series;

        leagueName = data.meta !== undefined ? data.meta.name : ("League " + g.lid);

		if (g.gameType == 1) {
			fileName = "CBBCOACH320_" + leagueName.replace(/[^a-z0-9]/gi, '_') + "_" + g.season + "_" + g.PHASE_TEXT[g.phase].replace(/[^a-z0-9]/gi, '_');					
		} else {
			fileName = "CBBCOACH80_" + leagueName.replace(/[^a-z0-9]/gi, '_') + "_" + g.season + "_" + g.PHASE_TEXT[g.phase].replace(/[^a-z0-9]/gi, '_');
		}		
        ...
        if (g.phase === g.PHASE.PLAYOFFS && data.hasOwnProperty("playoffSeries")) {
            ...
            fileName += "_Conference_Round_" + (playoffSeries.currentRound + 1);
        ...
        if (g.phase === g.PHASE.PLAYOFFS64 && data.hasOwnProperty("playoffSeries64")) {
            console.log(data.playoffSeries64);
            // Most recent series info
            playoffSeries64 = data.playoffSeries64[data.playoffSeries64.length - 1];
            rnd = playoffSeries64.currentRound;
            fileName += "_National_Round_" + (playoffSeries64.currentRound + 1);
    ...
    function updateExportLeague(inputs, updateEvents) {
        ...
        if (updateEvents.indexOf("firstRun") >= 0) {
            categories = [
                ...
                {
                    objectStores: "schedule,playoffSeries,playoffSeries64",
                    name: "Schedule",
                    desc: "Current regular season schedule, CT games, and NT games.",
                    checked: true
                },
                {

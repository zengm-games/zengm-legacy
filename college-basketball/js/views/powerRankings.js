...
/*					teamSOS[i] = (teamOppWins[i]/(teamOppLosses[i]+teamOppWins[i])*2+teamOppOppWins[i]/(teamOppOppLosses[i]+teamOppOppWins[i]))/3;
                ...
                overallRankMetric = function (t) {
                    if (t.gp < 30) {
                        return t.powerRank * 3 * t.gp / 30 + t.performanceRank * 3 * t.gp / 30 + t.talentRank * 2 * (30 - t.gp) / 30;
                    }

                    return t.powerRank * 3 + t.performanceRank * 3 + t.talentRank * 2;
                };
...
//                teamConf = g.confs[t.tid].name;	
				var removeConference;
                for (i = 0; i < teams.length; i++) {
		//		console.log("teams[i].tid: "+teams[i].tid);
		//		console.log("teams[i].cid: "+teams[i].cid);				
		//		console.log("teams[i].did: "+teams[i].did);				
		//		console.log(g.confs[teams[i].cid].name);
            ...
            //        teams[i].teamConf = teams[i].teamConf.replace('Conference','');
                }
//                return dao.teams.put({ot: tx, value: t}).then(function () {
        //        return dao.teams.put({ot: tx, value: teams}).then(function () {
		//		console.log(g.confs[0].name);
			//	console.log(g.confs[1].name);				
					return {
						teams: teams
					};
    ...
    function uiFirst(vm) {
        ...
        ko.computed(function () {
            ui.datatableSinglePage($("#power-rankings"), 0, _.map(vm.teams(), function (t) {
                ...
                return [String(t.overallRank), performanceRank, String(t.talentRank), SOSRank, RPIRank, powerRank, String(t.numPlayers), '<a href="' + helpers.leagueUrl(["roster", t.abbrev]) + '">' + t.region + ' ' + t.name + '</a>',String(t.teamConf),String(t.state), String(t.won), String(t.lost), t.lastTen, helpers.round(t.diff, 1), t.tid === g.userTid];
            }), {

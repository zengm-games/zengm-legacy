...
define(["dao", "globals", "ui", "core/draft", "core/finances", "core/player", "lib/bluebird", "lib/jquery", "lib/knockout", "lib/underscore", "util/bbgmView", "util/helpers"], function (dao, g, ui, draft, finances, player, Promise, $, ko, _, bbgmView, helpers) {
    ...
    function addSeason(season, tid) {
        ...
        return dao.players.getAll({
            ...
            playersAll = player.filter(playersAll, {
                attrs: ["pid", "tid", "draft","name", "pos", "age", "watch", "valueFuzz"],
            ...
            for (i = 0; i < playersAll.length; i++) {
                ...
                // Attributes
                p = {pid: pa.pid, name: pa.name, pos: pa.pos, age: pa.age, watch: pa.watch, valueFuzz: pa.valueFuzz, tid: pa.tid, dy: pa.draft.year};
    ...
    function updateDraftScouting(inputs, updateEvents) {
        ...
        if (updateEvents.indexOf("firstRun") >= 0 || updateEvents.indexOf("dbChange") >= 0) {
			...
			/*console.log(g.season+" "+seasonOffset+" "+firstUndraftedTid);
			console.log(g.season+" "+seasonOffset+" "+g.PLAYER.UNDRAFTED);			
			console.log(g.season+" "+seasonOffset+" "+(seasonOffset+1)+" "+g.PLAYER.UNDRAFTED_2);
			console.log(g.season+" "+seasonOffset+" "+(seasonOffset+2)+" "+g.PLAYER.UNDRAFTED_3);*/
            return Promise.all([
            ...
            dao.teams.get({key: g.userTid}).then(function (t) {
                ...
                dao.players.iterate({
                    ...
                    players.forEach(function (p) {
						...
						createUndrafted = Math.round(150 * g.numTeams / 30);
                    // "Top off" the draft class if <70 players imported
                    if (players.length < createUndrafted) {
                        draft.genPlayers(tx, draftClassTid, scoutingRank, createUndrafted - players.length,null);
                    }
                });
...
//                    return [String(p.rank), helpers.playerNameLabels(p.pid, p.name, undefined, p.skills, p.watch), p.pos, String(p.age), String(p.ovr), String(p.pot)];
                    return [String(p.rank), helpers.playerNameLabels(p.pid, p.name, undefined, p.skills, p.watch), p.pos, String(p.ovr), String(p.pot)];
//                    return [String(p.rank),  p.pos, String(p.ovr), String(p.pot), String(p.tid), String(p.dy)];
                }));
            }
        }).extend({throttle: 1});

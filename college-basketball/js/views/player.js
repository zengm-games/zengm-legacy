    ...
    function updatePlayer(inputs, updateEvents, vm) {
        if (updateEvents.indexOf("dbChange") >= 0 || updateEvents.indexOf("firstRun") >= 0 || !vm.retired()) {
            return Promise.all([
                ...
                p = player.filter(p, {
                    attrs: ["pid", "name", "tid", "abbrev", "teamRegion", "teamName", "pos", "age","year", "hgtFt", "hgtIn", "weight", "born", "contract", "draft", "face", "mood", "injury", "salaries", "salariesTotal", "awardsGrouped", "freeAgentMood", "imgURL", "watch","city","state","miles", "gamesUntilTradable"],
                    ratings: ["season", "abbrev", "age", "year","ovr", "pot", "hgt", "stre", "spd", "jmp", "endu", "ins", "dnk", "ft", "fg", "tp", "blk", "stl", "drb", "pss", "reb", "skills"],
                    stats: ["season", "abbrev", "age","year", "gp", "gs", "min", "fg", "fga", "fgp", "fgAtRim", "fgaAtRim", "fgpAtRim", "fgLowPost", "fgaLowPost", "fgpLowPost", "fgMidRange", "fgaMidRange", "fgpMidRange", "tp", "tpa", "tpp", "ft", "fta", "ftp", "orb", "drb", "trb", "ast", "tov", "stl", "blk", "pf", "pts", "per", "ewa"],
                    playoffs: true,
                    showNoStats: true,
                    showRookies: true,
                    fuzz: true
		...
		/*		console.log("p.salaries.amount : "+p.salaries.amount);
				console.log("p.salaries : "+p.salaries);
				console.log("p.salaries[0] : "+p.salaries[0]);
				console.log("p.salaries.amount[0] : "+p.salaries.amount[0]);*/

                // Account for extra free agent demands
                if (p.tid === g.PLAYER.FREE_AGENT) {
                    p.contract.amount = freeAgents.amountWithMoodMiles(p.contract.amount, p.freeAgentMood[g.userTid],p.miles[g.userTid]);
                }
...
console.log(feats);
                ...
                return {
                    ...
                    freeAgent: p.tid === g.PLAYER.FREE_AGENT && g.phase === g.PHASE.FREE_AGENCY,

    ...
    function create(tx, pid, resigning, tid) {
        ...
        return lock.canStartNegotiation(tx).then(function (canStartNegotiation) {
            ...
            return dao.players.count({
				...
				return dao.teams.get({ot: tx, key: g.userTid}).then(function (t) {
...
//                            text: 'The ' + g.teamRegionsCache[g.userTid] + ' ' + g.teamNamesCache[g.userTid] + ' re-signed <a href="' + helpers.leagueUrl(["player", p.pid]) + '">' + p.name + '</a> for ' + helpers.formatCurrency(p.contract.amount / 1000, "M") + '/year through ' + p.contract.exp + '.',
                            text: 'The <a href="' + helpers.leagueUrl(["roster", g.teamAbbrevsCache[g.userTid], g.season]) + '">' + g.teamNamesCache[g.userTid] + '</a> re-signed <a href="' + helpers.leagueUrl(["player", p.pid]) + '">' + p.name + '</a>.',
...
//                            text: 'The ' + g.teamRegionsCache[g.userTid] + ' ' + g.teamNamesCache[g.userTid] + ' signed <a href="' + helpers.leagueUrl(["player", p.pid]) + '">' + p.name + '</a> for ' + helpers.formatCurrency(p.contract.amount / 1000, "M") + '/year through ' + p.contract.exp + '.',
                            text: 'The <a href="' + helpers.leagueUrl(["roster", g.teamAbbrevsCache[g.userTid], g.season]) + '">' + g.teamNamesCache[g.userTid] + '</a> signed <a href="' + helpers.leagueUrl(["player", p.pid]) + '">' + p.name + '</a>.',

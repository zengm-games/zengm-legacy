...
define(["dao", "globals", "core/player", "lib/bluebird", "lib/underscore", "util/eventLog", "util/helpers", "util/random"], function (dao, g, player, Promise, _, eventLog, helpers, random) {
    ...
    function addSeasonRow(t) {
        ...
        newSeason = {
            ...
            playoff64RoundsWon: -1,  // -1: didn't make playoffs. 0: lost in first round. ... 6: won championship
            hype: Math.random(),
			//hype: (1- t.tid/400-.1),			
     ...
     * @return {Object} Updated team object.
     */
    function addStatsRow(t, playoffs,playoffs64) {
        var i;

        playoffs = playoffs !== undefined ? playoffs : false;
        playoffs64 = playoffs64 !== undefined ? playoffs64 : false;
        ...
        // If there is already an entry for this season+playoffs, do nothing
        for (i = 0; i < t.stats.length; i++) {
            if (t.stats[i].season === g.season && t.stats[i].playoffs64 === playoffs64) {
                return t;
            }
        }
    ...
    function rosterAutoSort(tx, tid) {
        ...
        return dao.players.getAll({
            ...
            for (i = 0; i < players.length; i++) {
				/*if (tid == 0) {
					console.log(tid+" "+players[i].valueNoPot+" "+players[i].valueNoPotFuzz)					
				}*/
    ...
    */
    function getContracts(tx, tid) {
        ...
        return dao.players.getAll({
            ...
            contracts = [];
            for (i = 0; i < players.length; i++) {
			//    console.log("i: "+i+" players[i].contract.amount: "+players[i].contract.amount);
                contracts.push({
                    pid: players[i].pid,
                    name: players[i].name,
                    ...
                    cre: players[i].contract.cre,
                    released: false
                });
            }

          //  console.log(" contracts.yearsWithTeam: "+contracts.exp);
			...
			//console.log(releasedPlayers.length);
            ...
            return Promise.map(releasedPlayers, function (releasedPlayer) {
                return dao.players.get({
					...
					//	console.log(g.season);													
						if (releasedPlayer.contract.cre == g.season || ( (releasedPlayer.contract.cre == g.season-1) && (g.phase == 0) ) ) {
					//	console.log(releasedPlayer);								
					//	console.log(g.phase);							
							contracts.push({
								...
								cre: releasedPlayer.contract.cre,								
								released: true
							});
						}
                    } else {
    ...
    function getPayroll(tx, tid) {
		...
		//	    console.log("i: "+i+" contracts[i].amount: "+contracts[i].amount);
                payroll += contracts[i].amount*1;  // No need to check exp, since anyone without a contract for the current season will not have an entry
		//	    console.log("i: "+i+" payroll: "+payroll);
            }
...
if (arguments[1] !== undefined) { throw new Error("No cb should be here"); }
        ...
        options.playoffs64 = options.playoffs64 !== undefined ? options.playoffs64 : false;
...
//if (tsa.revenues === undefined) { debugger; }
                ...
                for (j = 0; j < options.seasonAttrs.length; j++) {
                    if (options.seasonAttrs[j] === "winp") {
                    ...
                    } else if (options.seasonAttrs[j] === "winpConf") {
                        ft.winpConf = 0;
                        if ( (tsa.wonConf+tsa.lostConf) > 0) {
                            ft.winpConf = tsa.wonConf / (tsa.wonConf+tsa.lostConf);
                        }
                        ...
                        ft.cash = tsa.cash*1;  // [millions of dollars]
                    } else if (options.seasonAttrs[j] === "revenue") {
                        ft.revenue = tsa.revenue;  // [millions of dollars]
                    } else if (options.seasonAttrs[j] === "profit") {
                        ft.profit = (tsa.revenue - tsa.expense);  // [millions of dollars]
                    } else if (options.seasonAttrs[j] === "salaryPaid") {
                        ft.salaryPaid = tsa.expenses.salary.amount;  // [millions of dollars]
        ...
        filterStats = function (ft, t, options) {
            ...
            if (options.stats.length > 0) {
                if (options.season !== null) {
                    ...
                    for (j = 0; j < t.stats.length; j++) {
                        if (t.stats[j].season === options.season && t.stats[j].playoffs64 === options.playoffs64) {
                            ts = t.stats[j];
                            break;
                        }
                    }					
                    ...
                    ts = [];
                    for (j = 0; j < t.stats.length; j++) {
                        if (t.stats[j].playoffs64 === options.playoffs64) {
                            ts.push(t.stats[j]);
                        }
                    }					
                }
        ...
        return dao.teams.getAll({ot: options.ot, key: options.tid}).then(function (t) {
            ...
            } else if (options.sortBy === "winpConf") {
                // Sort by winning percentage, descending
                fts.sort(function (a, b) { return b.winpConf - a.winpConf; });
           /* } else if (options.sortBy === "power") {
                // Sort by winning percentage, descending
                fts.sort(function (a, b) { return b.winpConf - a.winpConf + b.winp - a.winp; });*/
            }
                        ...
                        for (i = 0; i < dpidsRemove.length; i++) {
                            dao.draftPicks.get({ot: tx, key: dpidsRemove[i]}).then(function (dp) {
                                ...
                                remove.push({
                                    ...
                                    skills: [],
                                    contract: {
                                        amount: rookieSalaries[estPick - 1 + g.numTeams * (dp.round - 1)] / 1,
                                        exp: dp.season + 2 + (2 - dp.round) // 3 for first round, 2 for second
                                    },
                                    worth: {
                                        amount: rookieSalaries[estPick - 1 + g.numTeams * (dp.round - 1)] / 1,
...
/*            // Handle situations where the team goes over the roster size limit
            ...
            sumValues = function (players, includeInjuries) {
                ...
                exponential = _.reduce(players, function (memo, p) {
                    ...
                    contractValue = (p.worth.amount - p.contract.amount) / 1;
...
//console.log([playerValue, contractValue]);
            ...
            sumContracts = function (players, onlyThisSeason) {
                ...
                sum = _.reduce(players, function (memo, p) {
                    ...
                    return memo + p.contract.amount / 1 * Math.pow(player.contractSeasonsRemaining(p.contract.exp, 82 - gpAvg), 0.25 - (onlyThisSeason ? 0.25 : 0));
                }, 0);
            ...
            return dao.players.getAll({ot: tx, index: "tid", key: tid}).then(function (players) {
                ...
                if (numPlayersOnRoster > 13) {
					if (g.userTids.indexOf(tid) >= 0  && g.autoPlaySeasons === 0) {
						...
						userTeamSizeError += 'more than the maximum number of players (13). You must remove players (by <a href="' + helpers.leagueUrl(["roster"]) + '">releasing them from your roster</a> or through <a href="' + helpers.leagueUrl(["trade"]) + '">trades</a>) before continuing.';
					
//                    if (tid === g.userTid && g.autoPlaySeasons === 0) {
  //                      userTeamSizeError = 'Your team currently has more than the maximum number of players (13). You must remove players (by <a href="' + helpers.leagueUrl(["roster"]) + '">releasing them from your roster before continuing.';
                    } else {
                        // Automatically drop lowest value players until we reach 15
                        players.sort(function (a, b) { return a.value - b.value; }); // Lowest first
                        for (i = 0; i < (numPlayersOnRoster - 13); i++) {
            ...
            //        if (tid === g.userTid && g.autoPlaySeasons === 0) {
                 //       userTeamSizeError = 'Your team currently has less than the minimum number of players (' + g.minRosterSize + '). You must add players (through <a href="' + helpers.leagueUrl(["free_agents"]) + '">recruiting</a> or use tools to create players before continuing.';
                    } else {
                        // Auto-add players
//console.log([tid, minFreeAgents.length, numPlayersOnRoster]);
                        while (numPlayersOnRoster < g.minRosterSize) {
                            ...
                            p.tid = tid;
                            p = player.addStatsRow(tx, p, g.phase === g.PHASE.PLAYOFFS64);
//                            p = player.addStatsRow(tx, p, g.phase === g.PHASE.PLAYOFFS);
                            ...
                            eventLog.add(null, {
                                type: "freeAgent",
                                text: 'The <a href="' + helpers.leagueUrl(["roster", g.teamAbbrevsCache[p.tid], g.season]) + '">' + g.teamNamesCache[p.tid] + '</a> signed <a href="' + helpers.leagueUrl(["player", p.pid]) + '">' + p.name + '</a>.',

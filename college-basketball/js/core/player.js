...
//                amount /= 4; // Max $5 million/year
      ...
      ///////      }
		...
		//console.log(amount);
        return {amount: amount, exp: expiration, cre: g.season};
    }
	...
	//console.log(contract);
        ...
        if (signed) {
            ...
            for (i = start; i <= p.contract.exp; i++) {
			    if ( (i==start) && ((start)> g.startingSeason) ) {
...
//                        fp.teamName = "Recruiting Prospect";
    ...
    function madeHof(p, playerStats) {
        ...
        for (i = 0; i < mins.length; i++) {
            ...
            if ( (mins[i] < 10) && (mins.length == 1) ) {
				va = 0;
			} else {
				va = mins[i] * (pers[i] - prls[p.pos]*(2.5+g.gameType*.2)*1.0)/30 ;
			}
...
//            return 0.4 * potential + 0.6 * current;
    ...
    function updateValues(ot, p, ps) {
        ...
        /*    if (ps.length === 0 || (ps.length === 1 && ps[0].min < 2000)) {
			...
			/*if (p.tid==0) {
				console.log(p.tid+" "+p.valueNoPot+" "+p.valueNoPotFuzz)
			}*/
            p.value = value(p, ps);
    ...
    function retire(tx, p, playerStats) {
        eventLog.add(tx, {
            type: "retired",
            text: '<a href="' + helpers.leagueUrl(["player", p.pid]) + '">' + p.name + '</a>  has left.',
    ...
    function checkStatisticalFeat(tx, pid, tid, p, results) {
        ...
        if (p.stat.orb + p.stat.drb >= 8) {
            doubles += 1;
        }

        statArr = {};
        if (p.stat.pts >= 4 && p.stat.ast >= 4 && p.stat.stl >= 4 && p.stat.blk >= 4 && (p.stat.orb + p.stat.drb) >= 4) {
        ...
        if (doubles >= 3) {
            if (p.stat.pts >= 8) { statArr.points = p.stat.pts; }
            if (p.stat.orb + p.stat.drb >= 8) { statArr.rebounds = p.stat.orb + p.stat.drb; }
            if (p.stat.ast >= 8) { statArr.assists = p.stat.ast; }
            if (p.stat.stl >= 8) { statArr.steals = p.stat.stl; }
            if (p.stat.blk >= 8) { statArr.blocks = p.stat.blk; }
            saveFeat = true;
        }
        if (p.stat.pts >= 40) {
            statArr.points = p.stat.pts;
        ...
        if (saveFeat) {
            ...
            featText += '</a> in a ' + results.team[i].stat.pts + "-" + results.team[j].stat.pts + (won ? ' win over ' : ' loss to ') + g.teamRegionsCache[results.team[j].id] + '.';

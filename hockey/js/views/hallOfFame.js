        ...
        if (updateEvents.indexOf("dbChange") >= 0 || updateEvents.indexOf("firstRun") >= 0 || (updateEvents.indexOf("newPhase") >= 0 && g.phase === g.PHASE.BEFORE_DRAFT)) {
            return dao.players.getAll({
				...
				players = player.filter(players, {
					...
					ratings: ["ovr"],
					stats: ["season", "abbrev", "gp", "min", "trb", "ast", "pts", "fg","sfgsp","sfga","sfgs"]
				});
...
//						if (players[i].stats[j].gp * players[i].stats[j].min * players[i].stats[j].per > players[i].bestStats.gp * players[i].bestStats.min * players[i].bestStats.per) {
						if  (players[i].stats[j].pts+players[i].stats[j].sfgsp*players[i].stats[j].gp* players[i].stats[j].min  >  players[i].bestStats.pts+players[i].bestStats.sfgsp*players[i].bestStats.gp* players[i].bestStats.min  ) {
    ...
    function uiFirst(vm) {
        ...
        ko.computed(function () {
            ui.datatable($("#hall-of-fame"), 2, _.map(vm.players(), function (p) {
                return ['<a href="' + helpers.leagueUrl(["player", p.pid]) + '">' + p.name + '</a>', p.pos, String(p.draft.year), String(p.retiredYear), String(p.peakOvr), String(p.bestStats.season),  '<a href="' + helpers.leagueUrl(["roster", p.bestStats.abbrev, p.bestStats.season]) + '">' + p.bestStats.abbrev + '</a>', String(p.bestStats.gp), helpers.round(p.bestStats.min, 1), helpers.round(p.bestStats.pts, 1), helpers.round(p.bestStats.fg, 1), helpers.round(p.bestStats.ast, 1), helpers.round(p.bestStats.sfgsp, 1), String(p.careerStats.gp), helpers.round(p.careerStats.min, 1), helpers.round(p.careerStats.pts, 1), helpers.round(p.careerStats.fg, 1), helpers.round(p.careerStats.ast, 1), helpers.round(p.careerStats.sfgsp, 1), p.statsTids.indexOf(g.userTid) >= 0];
            }), {

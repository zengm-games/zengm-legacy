    ...
    function doRelease(pid, justDrafted) {
        ...
        return dao.players.count({
            ...
            if (numPlayersOnRoster <= 18) {
                return "You must keep at least 20 players on your roster. You currently only have 18.";
            }
...
//            return 0 - this.players.off.active().length;
            ...
            return count - this.players.off.active().length - this.players.def.active().length - this.players.off.inactive().length - this.players.def.inactive().length
        }, this);
...
//            return 23 - this.players.off.active().length - this.players.def.active().length
        }, this);
        this.numActiveSpots = ko.computed(function () {
            return 23 - this.players.off.active().length - this.players.def.active().length
    ...
    function updateRoster(inputs, updateEvents, vm) {
        ...
        if (updateEvents.indexOf("dbChange") >= 0 || (inputs.season === g.season && (updateEvents.indexOf("gameSim") >= 0 || updateEvents.indexOf("playerMovement") >= 0)) || inputs.abbrev !== vm.abbrev() || inputs.season !== vm.season()) {
            ...
            return team.filter({
                ...
                seasonAttrs: ["profit", "won", "lost","points", "overtime","playoffRoundsWon"],
                ot: tx
            }).then(function (t) {

...
//                    ratings: ["ovr", "pot", "hgt", "stre", "spd", "jmp", "endu", "ins", "dnk", "ft", "fg", "tp", "blk", "stl", "drb", "pss", "reb", "skills"],
                    ratings: ["ovr", "pot", "hgt", "stre", "spd", "jmp","quik", "endu", "ins","hsd", "dnk", "ft", "fg", "tp", "blk", "stl", "drb", "pss", "reb","face", "skills"],					
                    stats: ["abbrev"],
    ...
    function uiFirst(vm) {
        ...
        ko.computed(function () {
            ...
            ui.datatable($("#player-ratings"), 4, _.map(vm.players(), function (p) {
                return [helpers.playerNameLabels(p.pid, p.name, p.injury, p.ratings.skills, p.watch), p.pos, '<a href="' + helpers.leagueUrl(["roster", p.stats.abbrev, season]) + '">' + p.stats.abbrev + '</a>', String(p.age - (g.season - season)), String(p.ratings.ovr), String(p.ratings.pot), String(p.ratings.hgt), String(p.ratings.stre), String(p.ratings.spd), String(p.ratings.jmp), String(p.ratings.quik), String(p.ratings.endu), String(p.ratings.ins), String(p.ratings.hsd), String(p.ratings.dnk), String(p.ratings.ft), String(p.ratings.fg), String(p.ratings.tp), String(p.ratings.blk), String(p.ratings.stl), String(p.ratings.drb), String(p.ratings.pss), String(p.ratings.reb), String(p.ratings.face)];
            }));
        }).extend({throttle: 1});

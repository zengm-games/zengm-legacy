        ...
        if (updateEvents.indexOf("dbChange") >= 0 || updateEvents.indexOf("clearWatchList") >= 0 || updateEvents.indexOf("gameSim") >= 0 || updateEvents.indexOf("playerMovement") >= 0 || inputs.statType !== vm.statType() || inputs.playoffs !== vm.playoffs()) {
            ...
            return dao.players.getAll({
				...
				players = player.filter(players, {
					...
					stats: ["gp", "min", "fg","pts", "trb", "ast","sfgsp","sfga","sfgs"],
					season: g.season,
        ...
        ko.computed(function () {
            ...
            for (i = 0; i < vm.players().length; i++) {
                ...
                if (vm.playoffs() === "playoffs") {
                    ...
                    ["gp", "min", "fg","pts", "trb", "ast","sfgsp","sfga","sfgs"].forEach(function (category) {
                ...
                rows.push([helpers.playerNameLabels(p.pid, p.name, p.injury, p.ratings.skills, p.watch), p.pos, String(p.age), '<a href="' + helpers.leagueUrl(["roster", p.abbrev]) + '">' + p.abbrev + '</a>', String(p.ratings.ovr), String(p.ratings.pot), contract, String(p.stats.gp), helpers.round(p.stats.min, d), helpers.round(p.stats.pts, 0), helpers.round(p.stats.fg, 0), helpers.round(p.stats.ast, 0), helpers.round(p.stats.sfgsp, 1)]);
            }

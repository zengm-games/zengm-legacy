...
define(["globals", "lib/faces", "lib/knockout", "util/helpers"], function (g, faces, ko, helpers) {
    ...
    ko.bindingHandlers.recordAndPlayoffs = {
        update: function (element, valueAccessor) {
            var abbrev, args, extraText, lost, option, output, playoffRoundsWon, OT, points, season, won;
            ...
            OT = ko.unwrap(args[5]);
            points = ko.unwrap(args[6]);
            option = args.length > 7 ? ko.unwrap(args[7]) : null;
            ...
            output += '<a href="' + helpers.leagueUrl(["standings", season]) + '">' + won + '-' + lost + '-' + OT + ' (' + points+')</a>';
            if (extraText) {

...
define(["dao", "db", "globals", "ui", "core/draft", "core/finances", "core/phase", "core/player", "core/team", "lib/bluebird", "lib/jquery", "lib/underscore", "util/helpers", "util/random"], function (dao, db, g, ui, draft, finances, phase, player, team, Promise, $, _, helpers, random) {
     ...
     */
    function create(name, tid,leagueid,typeid, leagueFile, startingSeason, randomizeRosters) {
        var skipNewPhase, phaseText, teams, teamsDefault,teamsRussia,conferences,divisions;
		var salaryCap,minPayroll,luxuryPayroll,luxuryTax,minContract,maxContract;
        ...
        // Default teams
		if (leagueid == 0) {
			conferences = [{cid: 0, name: "Eastern Conference"}, {cid: 1, name: "Western Conference"}];
			divisions = [{did: 0, cid: 0, name: "Atlantic"}, {did: 1, cid: 0, name: "Metropolitan"}, {did: 2, cid: 1, name: "Central"}, {did: 3, cid: 1, name: "Pacific"}];			
			salaryCap = 73500;  // [thousands of dollars]
			minPayroll = 55800;  // [thousands of dollars]
			luxuryPayroll = 73500;  // [thousands of dollars]
			luxuryTax = 1.5;
			minContract = 575;  // [thousands of dollars]
			maxContract = 14280;  // [thousands of dollars]			
		} else {
			conferences = [{cid: 0, name: "Eastern  Conference"},{cid: 1, name: "Western  Conference"}];
			divisions = [ {did: 0, cid: 0, name: "Kharlamov"}, {did: 1, cid: 0, name: "Chernyshev"},{did: 2, cid: 1, name: "Bobrov"}, {did: 3, cid: 1, name: "Tarasov"}];
			salaryCap = 36500;  // [thousands of dollars]
			minPayroll = 0;  // [thousands of dollars]
			luxuryPayroll = 36500;  // [thousands of dollars]
			luxuryTax = 1.5;
			minContract = 160;  // [thousands of dollars]
			maxContract = 14280;  // [thousands of dollars]			
		}
		
        teamsDefault = helpers.getTeamsDefault();
        teamsRussia = helpers.getTeamsRussia();
        // Any custom teams?
        if (leagueFile.hasOwnProperty("teams")) {
			if (leagueid == 0) {
				teams = merge(leagueFile.teams, teamsDefault);
			} else {
				teams = merge(leagueFile.teams, teamsRussia);
			}		
            ...
            if (leagueFile.hasOwnProperty("draftPicks")) {
                ...
                for (i = 0; i < 4; i++) {
                    for (t = 0; t < g.numTeams; t++) {
                                for (round = 1; round <= (2+g.gameType); round++) {
                             dao.draftPicks.add({
                                ot: tx,
                                value: {
                                    tid: t,
...
//                        var afterPlayerCreation, agingYears, baseRatings, cbAfterEachPlayer,  draftYear, goodNeutralBad, i, j, n, numLeft, p, players, pots, profile, profiles, randomizeExpiration, t, t2, playerTids;
                     ...
                     /*   afterPlayerCreation = function () {
                            var createUndrafted1, createUndrafted2, createUndrafted3, i, draftSize;
                           ...
                           tx = g.dbl.transaction(["players", "playerStats"], "readwrite");
                            ...
                            createUndrafted1 = Math.round(draftSize * g.numTeams / 30);
                            createUndrafted2 = Math.round(draftSize * g.numTeams / 30);
                            createUndrafted3 = Math.round(draftSize * g.numTeams / 30);
            ...
            // If the draft has already happened this season but next year's class hasn't been bumped up, don't create any g.PLAYER.UNDRAFTED
                            ...
                            tx.oncomplete = function () {
                                if (skipNewPhase) {
                                    ...
                                    phase.newPhase(g.PHASE.REGULAR_SEASON).then(function () {
                                        var lid;
...
//                            profiles = ["Point", "Wing", "Big", ""];
							...
							if (g.leagueType == 0) {	
								adj = 0;								
								baseRatings = [37-adj, 37-adj, 36-adj, 35-adj, 34-adj, 33-adj, 32-adj, 31-adj, 30-adj, 29-adj, 28-adj, 26-adj, 26-adj, 26-adj, 31-adj, 30-adj, 29-adj, 28-adj, 26-adj, 26-adj, 26-adj,32];
								adj = 0;								
								pots = [75-adj, 65-adj, 55-adj, 55-adj, 60-adj, 50-adj, 70-adj, 40-adj, 55-adj, 50-adj, 60-adj, 60-adj, 45-adj, 45-adj, 60-adj, 50-adj, 70-adj, 40-adj, 55-adj, 50-adj, 60-adj, 60-adj, 45-adj, 45];
							} else {
								adj = 0;									
								baseRatings = [27-adj, 27-adj, 26-adj, 25-adj, 24-adj, 23-adj, 22-adj, 21-adj, 20-adj, 19-adj, 18-adj, 16-adj, 16-adj, 16-adj, 21-adj, 20-adj, 19-adj, 18-adj, 16-adj, 16-adj, 16-adj,22];
								adj = 0								
								pots = [65-adj, 55-adj, 45-adj, 45-adj, 50-adj, 40-adj, 60-adj, 30-adj, 45-adj, 40-adj, 50-adj, 50-adj, 35-adj, 35-adj, 50-adj, 40-adj, 60-adj, 30-adj, 45-adj, 40-adj, 50-adj, 50-adj, 35-adj, 35];
							}

//                            numLeft = 33 * 14;
							if (g.gameType == 0) {
								numPlayers = 23+0; // Total number allowed
							} else if (g.gameType == 1) {
								numPlayers = 23+5; // Total number allowed
							} else if (g.gameType == 2) {
								numPlayers = 23+10; // Total number allowed
							} else if (g.gameType == 3) {
								numPlayers = 23+15; // Total number allowed
							} else if (g.gameType == 4) {
								numPlayers = 23+20; // Total number allowed
							} else {
								numPlayers = 23+23; // Total number allowed
							}

							while (baseRatings.length < numPlayers) {
								baseRatings.push(g.leagueType*10+16);
							}
							while (pots.length < numPlayers) {
								pots.push(g.leagueType*10+35);
							}							
...
//                                for (n = 0; n < 14; n++) {
                                for (n = 0; n < numPlayers; n++) {
									...
									if (t < 0 ) {
										agingYears = random.randInt(5, 13);
										draftYear = g.startingSeason - 1 - agingYears;
//										p = player.generate(t2, 19, profile, baseRatings[n]+g.gameType*1, pots[n]+g.gameType*1, draftYear, true, scoutingRank);
										p = player.generate(t2, 19, profile, 27+g.gameType*1, 30+g.gameType*1, draftYear, true, scoutingRank);
																				
									} else if (n> (22+g.gameType)) {
										agingYears = random.randInt(0, 4);
										draftYear = g.startingSeason - 1 - agingYears;
										p = player.generate(t2, 19, profile, baseRatings[n]-g.gameType*3-10, pots[n]-g.gameType*5, draftYear, true, scoutingRank);
           ...
           //         }
                ...
                var createUndrafted1, createUndrafted2, createUndrafted3, i, tx,draftSize;

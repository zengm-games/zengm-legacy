...
define(["db", "globals", "ui", "core/contractNegotiation", "core/draft", "core/finances", "core/freeAgents", "core/player", "core/team", "lib/jquery", "lib/underscore", "util/account", "util/eventLog", "util/helpers", "util/message", "util/random"], function(db, g, ui, contractNegotiation, draft, finances, freeAgents, player, team, $, _, account, eventLog, helpers, message, random) {
	...
	function updateOwnerMood(cb) {
		team.filter({
			...
			if (t.playoffRoundsWon < 0) {
				deltas.playoffs = -0.2;
			} else if (t.playoffRoundsWon < 2) {
				deltas.playoffs = 0.06 * t.playoffRoundsWon + .04;
			} else {
				deltas.playoffs = 0.2;
			}
		...
		team.filter({
			...
			tx.objectStore("players").index("tid").getAll(IDBKeyRange.lowerBound(g.PLAYER.FREE_AGENT)).onsuccess = function(event) {
				...
				players.sort(function(a, b) {
					return b.stats.orb - a.stats.orb + b.stats.drb - a.stats.drb + b.stats.stl - a.stats.stl;
				});
				// Same formula as MVP, but no wins because some years with bad rookie classes can have the wins term dominate EWA
				for ( i = 0; i < players.length; i++) {
				...
				players.sort(function(a, b) {
					return (b.stats.qbr + 1.0 * b.won * b.gs / 16) - (a.stats.qbr + 1.0 * a.won * b.gs / 16);
				});
				p = players[0];
				awards.mvp = {
					pid : p.pid,
				...
				numDone = 0;
				players.sort(function(a, b) {
					return b.stats.olr * 4 + b.stats.orb - a.stats.orb + b.stats.drb - a.stats.olr * 4 - a.stats.drb + b.stats.stl - a.stats.stl;
				});
				// Same formula as MVP, but no wins because some years with bad rookie classes can have the wins term dominate EWA

				awards.allLeague = [{
					title : "All Pro - Offense",
					players : []
				}];
				...
				players.sort(function(a, b) {
					return (b.stats.fgaMidRange) - (a.stats.fgaMidRange) + (b.stats.dec - b.stats.der * 15 - b.stats.dep * 5) - (a.stats.dec - a.stats.der * 15 - a.stats.dep * 5);
				});
				...
				if ((p.pos == "S") || (p.pos == "CB") || (p.pos == "LB")) {
					awards.dpoy = {
						...
						blk : p.stats.blk,
						stl : p.stats.stl,
						fgaMidRange : p.stats.fgaMidRange,
						intery : p.stats.intery,
						derpatp : p.stats.derpatp
					};
					awardsByPlayer.push({
						pid : p.pid,
						tid : p.tid,
					...
					players.sort(function(a, b) {
						return (b.stats.fgaMidRange) * 10 - (a.stats.fgaMidRange) * 10 - (b.stats.der) + (a.stats.der);
					});
				...
				tx.objectStore("players").index("tid").getAll(champTid).onsuccess = function(event) {
					...
					players.sort(function(a, b) {
						return b.statsPlayoffs.orb - a.statsPlayoffs.orb + b.statsPlayoffs.drb - a.statsPlayoffs.drb + b.statsPlayoffs.stl - a.statsPlayoffs.stl;
					});
					p = players[0];
					awards.finalsMvp = {
						pid : p.pid,
						...
						tid : p.tid,
						abbrev : p.abbrev,
						pts : p.statsPlayoffs.pts,
						trb : p.statsPlayoffs.trb,
						ast : p.statsPlayoffs.ast,
						stl : p.statsPlayoffs.stl,
						drb : p.statsPlayoffs.drb,
						orb : p.statsPlayoffs.orb
					};
					awardsByPlayer.push({
						pid : p.pid,
						tid : p.tid,
	...
	function newScheduleCrappy() {
		...
		numWithRemaining = g.numTeams;
		// Number of teams with numRemaining > 0
			...
			for ( cid = 0; cid < 2; cid++) {
				...
				for ( i = 0; i < teams.length; i++) {
					if (teams[i].cid === cid) {
						...
						if ((teamsConf.length < 3) && (divTeamRank[i] < 2)) {
				...
				for ( i = 0; i < teams.length; i++) {
					if (teams[i].cid === cid) {
						//                        if (teamsConf.length < 8) {
						//                        if (teamsConf.length < 6) {
						if ((teamsConf.length < 3) && (divTeamRank[i] > 1)) {
				...
				series[1][cid].home.seed = 3;
				series[1][cid].away.seed = 2;

				series[2][cid] = {
					home : teamsConf[0],
					away : teamsConf[0]
				};
				series[2][cid].home.seed = 1;
				...
				if (cursor) {
					...
					if (age > maxAge || pot < minPot) {
						...
						if (age > 34 || p.tid === g.PLAYER.FREE_AGENT) {// Only players older than 34 or without a contract will retire
							if (age > 34) {
								excessAge = (age - 34) / 20;
								// 0.05 for each year beyond 34
							}
							excessPot = (40 - pot) / 50;
							// 0.02 for each potential rating below 40 (this can be negative)
							...
							if (p.contract.exp <= g.season) {
								if (p.tid !== g.userTid) {
									...
									if (Math.random() < player.value(p) / 100 - factor) {// Should eventually be smarter than a coin flip
										...
										contract.exp += 1;
										// Otherwise contracts could expire this season
										p = player.setContract(p, contract, true);
										p.gamesUntilTradable = 15;
										cursor.update(p);
										// Other endpoints include calls to addToFreeAgents, which handles updating the database
									} else {
			...
			if (tids.length > 0) {
				...
				if (rnd === 3) {
					...
					for ( i = 0; i < 1; i += 2) {
						...
						teamNumberOneC1 = helpers.deepCopy(series[2][0].home);
						teamNumberOneC2 = helpers.deepCopy(series[2][1].home);
						...
						//// works
						series[rnd+1][0] = {
							home : teamNumberOneC1,
							away : team0
						};
						//// error here :Uncaught TypeError: Cannot read property '0' of undefined

						///////						series[rnd+1][0]  = {home: topseeds[0], away: team0};                //// error here :Uncaught TypeError: Cannot read property '0' of undefined
						//series[0][cid * 4] = {home: teamsConf[0], away: teamsConf[7]};

						series[2][0].home.seed = 1;
						//						series[1][0].away.seed = 15;
						series[2][0].away.seed = seed0;
						series[2][0].home.won = 0
						series[2][0].away.won = 0

						//// doesn't work

						//	teamNumberOneC2
						series[rnd+1][1] = {
							home : teamNumberOneC2,
							away : team1
						};
						...
						////////////////////////////						series[rnd+1][1]  = {home: topseeds[1], away:  topseeds[2] };
						//series[0][cid * 4] = {home: teamsConf[0], away: teamsConf[7]};
						series[2][1].home.seed = 1;
						////						series[1][2].away.seed = 30;
						series[2][1].away.seed = seed1;
						series[2][1].home.won = 0
						series[2][1].away.won = 0
					...
					//                    for (i = 0; i < series[rnd].length; i += 2) {
					//                    for (i = 0; i < series[rnd].length; i += 2) {
					//for (i = 0; i < series[rnd].length; i += 10) {
					i = 0;
	...
	function getSchedule(ot, numDays, cb) {
		...
		scheduleStore.getAll().onsuccess = function(event) {
			...
			if (numDays > 0) {
				schedule = schedule.slice(0, g.numTeams / 2);
				// This is the maximum number of games possible in a day

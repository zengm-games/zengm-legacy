...
//console.log('writeTeamStats 2');
            tx.objectStore("teams").openCursor(that.team[t1].id).onsuccess = function (event) {
                ...
                att = 10000 + (0.1 + 0.9 * Math.pow(teamSeason.hype, 2)) * teamSeason.pop * 500000 * 0.01;  // Base attendance - between 2% and 0.2% of the region
                if (that.playoffs) {
                ...
                if (!that.playoffs) {
                    ...
                    salaryPaid = payroll / 18;
                    scoutingPaid = t.budget.scouting.amount / 18;
                    coachingPaid = t.budget.coaching.amount / 18;
                    healthPaid = t.budget.health.amount / 18;
                    facilitiesPaid = t.budget.facilities.amount / 18;
                    merchRevenue = 3 * att / 1000*82/18*5;
                    if (merchRevenue > 250*82/18*5) {
                        merchRevenue = 250*82/18*5;
                    }
                    sponsorRevenue = 10 * att / 1000*82/18*5;
                    if (sponsorRevenue > 600*82/18*5) {
                        sponsorRevenue = 600*82/18*5;
                    }
                    nationalTvRevenue = 125*82/18*15;
                    localTvRevenue = 10 * att / 1000*82/18*5;
                    if (localTvRevenue > 1200*82/18*5) {
                        localTvRevenue = 1200*82/18*5;
                    }
                }
                ...
                att += 15000;  // Attendance depends on facilities. Not sure if this formula is reasonable.
                if (att > 90000) {
                    att = 90000;
                } else if (att < 0) {
                    att = 0;

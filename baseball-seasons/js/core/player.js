...
//        amount = ((p.value - 1) / 100 - 0.60) * 1.5 * (maxAmount - minAmount) + minAmount;
		...
		amount = ((p.value - 1) / 100 - 0.50) * 3.2 * (maxAmount - minAmount) + minAmount;
        if (randomizeAmount) {
...
//            p.ratings[r][key] = limitrating.limitRating(p.ratings[r][key] + amount);
    ...
    function generate(tid, age, profile, baseRating, pot, draftYear, newLeague, scoutingRank) {
        ...
        if (newLeague) {
            ...
            p.ratings.push(playerSport.genRatings(profile, baseRating, pot, g.startingSeason, scoutingRank));
        } else {
            // Create player to be drafted
            p.ratings.push(playerSport.genRatings(profile, baseRating, pot, draftYear, scoutingRank));
        }
...
//					   row[stats[j]] = s[stats[j]] * 9 /  s["fta"];
					   ...
					   if (s.fga==0) {
						row[stats[j]] = 0;
						  ...
						  row[stats[j]] = (s.ft*2 + s.orb*3 + s.blk*4) / (s.fga) ;

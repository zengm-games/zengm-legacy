    ...
    function updateTeamHistory(inputs, updateEvents, vm) {
        if (updateEvents.indexOf("dbChange") >= 0 || updateEvents.indexOf("firstRun") >= 0 || updateEvents.indexOf("gameSim") >= 0 || inputs.abbrev !== vm.abbrev()) {
            return Promise.all([
                ...
                var   championships, history, i, playoffAppearances, totalWon, totalLost, j,userTeam, userTeamSeason;
                ...
                history.reverse(); // Show most recent season first
                    ...
                    players = player.filter(players, {
						...
						ratings: ["ovr", "skills"],					
                        stats: ["gp", "min", "pts", "trb", "ast", "per", "ewa","fga","fta","blk","pf","winP","lossP","save","war","stl"],
                        tid: inputs.tid
                    });
...
//                return [helpers.playerNameLabels(p.pid, p.name, p.injury, [], p.watch), p.pos,, String(p.peakOvr), p.hof, p.tid > g.PLAYER.RETIRED && p.tid !== vm.team.tid(), p.tid === vm.team.tid()];

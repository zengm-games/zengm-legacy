    ...
    function updateTrade(inputs, updateEvents, vm) {
        ...
        validateSavedPids(function (teams) {
            ...
            playerStore.index("tid").getAll(g.userTid).onsuccess = function (event) {
                ...
                stats = ["min", "pts", "trb", "ast", "per","fga","blk","fta","fgAtRim","war"];
...
//                checkbox = '<input name="' + userOrOther + '-pids" type="checkbox" value="' + p.pid + '" title="' + p.untradableMsg + '"' + selected + disabled + '>';
                ...
                return [checkbox, helpers.playerNameLabels(p.pid, p.name, p.injury, p.ratings.skills, p.watch), p.pos, String(p.age), String(p.ratings.ovr), String(p.ratings.pot), String(p.ratings.hgt), String(p.ratings.stre), String(p.ratings.spd), String(p.ratings.jmp), String(p.ratings.endu), String(p.ratings.ins), String(p.ratings.dnk), String(p.ratings.ft), String(p.ratings.fg), String(p.ratings.tp), String(p.ratings.blk), String(p.ratings.stl), String(p.ratings.drb), String(p.ratings.pss), String(p.ratings.reb)];

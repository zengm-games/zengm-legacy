...
//        this.numPossessions = Math.round((this.team[0].pace + this.team[1].pace) / 2 * random.uniform(0.9, 1.1));
        this.numPossessions = Math.round((this.team[0].pace + this.team[1].pace) / 2 / 2 * 1.33 * random.uniform(0.9, 1.1));
  //      this.numPossessions = Math.round((this.team[0].pace + this.team[1].pace) / 2 / 2 * 1.59 * random.uniform(0.9, 1.1));
 //       this.dt = 48 / (2 * this.numPossessions); // Time elapsed per possession
        this.dt = 40 / (2 * this.numPossessions); // Time elapsed per possession
...
//        this.t = 10; // Game clock, in minutes
    ...
    //            this.numPossessions = Math.round(this.numPossessions * 5 / 48);  // 5 minutes of possessions
                this.numPossessions = Math.round(this.numPossessions * 5 / 40);  // 5 minutes of possessions
  ...
  //          if ((i * this.dt > 10 && this.team[0].stat.ptsQtrs.length === 1) ||
  //                  (i * this.dt > 20 && this.team[0].stat.ptsQtrs.length === 2) ||
  //                  (i * this.dt > 30 && this.team[0].stat.ptsQtrs.length === 3)) {
//            if ((i * this.dt > 12 && this.team[0].stat.ptsQtrs.length === 1) ||
 ...
 //                   (i * this.dt > 36 && this.team[0].stat.ptsQtrs.length === 2)) {
                this.team[0].stat.ptsQtrs.push(0);
                this.team[1].stat.ptsQtrs.push(0);
                this.t = 20;
//                this.t = 12;
//                this.t = 10;
    ...
    GameSim.prototype.updatePlayersOnCourt = function () {
		...
		//console.log(this.team[0].player[0].stat.energy);
        for (t = 0; t < 2; t++) {
            // Overall values scaled by fatigue
            ovrs = [];
            for (p = 0; p < this.team[t].player.length; p++) {
                // Injured or foulded out players can't play
                if (this.team[t].player[p].injured || this.team[t].player[p].stat.pf >= 5) {
                    ovrs[p] = -Infinity;
                } else {
					//if (this.team[t].player[p].stat.energy <.5) {
					//	console.log(this.team[t].player[p].stat.energy);
					//}
            ...
            for (pp = 0; pp < this.playersOnCourt[t].length; pp++) {
                ...
                for (b = 0; b < this.team[t].player.length; b++) {
                    if (this.playersOnCourt[t].indexOf(b) === -1 && ((this.team[t].player[p].stat.courtTime > 3 && this.team[t].player[b].stat.benchTime > 3 && ovrs[b] > ovrs[p]) || ((this.team[t].player[p].injured || this.team[t].player[p].stat.pf >= 5) && (!this.team[t].player[b].injured && this.team[t].player[b].stat.pf < 5)))) {
                        substitutions = true;
    ...
    GameSim.prototype.simPossession = function () {
        ...
        ratios = this.ratingArray("usage", this.o,1.5);
    ...
    GameSim.prototype.doStl = function (pStoleFrom) {
        ...
        ratios = this.ratingArray("stealing", this.d,8);
    ...
    GameSim.prototype.doShot = function (shooter) {
        ...
        if (this.probAst() > Math.random()) {
            ratios = this.ratingArray("passing", this.o, 4);
    ...
    GameSim.prototype.doPf = function (t) {
        ...
        // Foul out
        if (this.team[this.d].player[p].stat.pf >= 5) {
    ...
    GameSim.prototype.doReb = function () {
        ...
        if (0.75 * (2 + this.team[this.d].compositeRating.rebounding) / (2 + this.team[this.o].compositeRating.rebounding) > Math.random()) {
            ratios = this.ratingArray("rebounding", this.d,4);
            p = this.playersOnCourt[this.d][this.pickPlayer(ratios)];
        ...
        ratios = this.ratingArray("rebounding", this.o,4);
        p = this.playersOnCourt[this.o][this.pickPlayer(ratios)];
    ...
    GameSim.prototype.ratingArray = function (rating, t, power) {
        ...
        for (i = 0; i < 5; i++) {
            p = this.playersOnCourt[t][i];
            array[i] = Math.pow(this.team[t].player[p].compositeRating[rating] * this.fatigue(this.team[t].player[p].stat.energy/10), power*.75);
        }
    ...
    GameSim.prototype.recordPlay = function (type, t, names) {
        ...
        if (this.playByPlay !== undefined) {
            if (type === "injury") {
                ...
                texts = ["<b>Start of " + helpers.ordinal(this.team[0].stat.ptsQtrs.length) + " half</b>"];
            } else if (type === "overtime") {
                texts = ["<b>Start of " + helpers.ordinal(this.team[0].stat.ptsQtrs.length - 2) + " overtime period</b>"];
            } else if (type === "ft") {

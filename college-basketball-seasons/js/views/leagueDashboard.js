...
define(["dao", "globals", "ui", "core/player", "core/season", "core/team", "lib/knockout", "lib/underscore", "util/bbgmView", "util/helpers"], function (dao, g, ui, player, season, team, ko, _, bbgmView, helpers) {
        ...
        if (updateEvents.indexOf("dbChange") >= 0 || updateEvents.indexOf("firstRun") >= 0 || updateEvents.indexOf("gameSim") >= 0 || updateEvents.indexOf("playerMovement") >= 0 || updateEvents.indexOf("newPhase") >= 0) {
            return dao.teams.get({key: g.userTid}).then(function (t) {
                ...
                return {
                    ...
                    cash: latestSeason.cash / 1,  // [millions of dollars]
    ...
    function updateTeams(inputs, updateEvents) {
        ...
        if (updateEvents.indexOf("dbChange") >= 0 || updateEvents.indexOf("firstRun") >= 0 || updateEvents.indexOf("gameSim") >= 0 || updateEvents.indexOf("playerMovement") >= 0 || updateEvents.indexOf("newPhase") >= 0) {
            vars = {};
            stats = ["pts", "oppPts", "trb", "ast", "fga", "fgp", "tpa", "tpp", "fta", "ftp","ast", "tov", "stl", "blk", "pf"];  // This is also used later to find ranks for these team stats
            ...
            return team.filter({
                ...
                for (i = 0; i < teams.length; i++) {
                    if (teams[i].cid === cid) {
                        if (teams[i].tid === g.userTid) {
                            ...
                            vars.tpa = teams[i].tpa;
                            vars.tpp = teams[i].tpp;
                            vars.fta = teams[i].fta;
                            vars.ftp = teams[i].ftp;
                            vars.tov = teams[i].tov;
                            vars.stl = teams[i].stl;
                ...
                vars.oppPtsRank = 321 - vars.oppPtsRank;
                vars.pfRank = 321 - vars.pfRank;
                vars.tovRank = 321 - vars.tovRank;
        ...
        if (updateEvents.indexOf("dbChange") >= 0 || updateEvents.indexOf("firstRun") >= 0 || updateEvents.indexOf("gameSim") >= 0 || updateEvents.indexOf("playerMovement") >= 0 || updateEvents.indexOf("newPhase") >= 0) {
            ...
            return dao.players.getAll({
                ...
                players = player.filter(players, {
                    attrs: ["pid", "name", "abbrev", "tid", "age","year", "contract", "rosterOrder", "injury", "watch", "pos","city","state","miles"],
                    ...
                    for (rnd = playoffSeries.currentRound; rnd >= 0; rnd--) {
                        for (i = 0; i < series[rnd].length; i++) {
                            if (series[rnd][i].home.tid === g.userTid || series[rnd][i].away.tid === g.userTid) {
                               ...
                               if (rnd === 0) {
                                    vars.seriesTitle = "CT First Round";
                                } else if (rnd === 1) {
                                    vars.seriesTitle = "CT Second Round";
                                } else if (rnd === 2) {
                                    vars.seriesTitle = "CT Semi Finals";
                                } else if (rnd === 3) {
                                    vars.seriesTitle = "CT Finals";
                                }
                                break;
                            }
                        }
    ...
    function updateStandings(inputs, updateEvents) {
        if (updateEvents.indexOf("dbChange") >= 0 || updateEvents.indexOf("firstRun") >= 0 || updateEvents.indexOf("gameSim") >= 0) {
            return team.filter({
                ...
                seasonAttrs: ["wonConf", "lostConf","won", "lost", "winp","hype"],
                season: g.season,
                ...
                for (k = 0; k < teams.length; k++) {
                    if (cid === teams[k].cid) {
						teams[k].hype *= 100;
						teams[k].hype = Math.round(teams[k].hype);

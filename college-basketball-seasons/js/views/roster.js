    ...
    function updateRoster(inputs, updateEvents, vm) {
        ...
        if (updateEvents.indexOf("dbChange") >= 0 || (inputs.season === g.season && (updateEvents.indexOf("gameSim") >= 0 || updateEvents.indexOf("playerMovement") >= 0)) || inputs.abbrev !== vm.abbrev() || inputs.season !== vm.season()) {
            ...
            return team.filter({
                season: inputs.season,
                tid: inputs.tid,
                attrs: ["tid", "region", "name", "strategy", "imgURL","city","state"],
                seasonAttrs: ["profit", "cash","won", "lost", "playoffRoundsWon", "playoff64RoundsWon"],
                ot: tx
            }).then(function (t) {
                ...
                attrs = ["pid", "tid", "draft", "name", "pos","year", "age", "contract", "cashOwed", "rosterOrder", "injury", "ptModifier", "watch","city","state","miles"];  // tid and draft are used for checking if a player can be released without paying his salary
//                ratings = ["ovr", "pot", "dovr", "dpot", "skills"];
				ratings  = ["ovr", "pot", "hgt","stre","spd","jmp","endu","ins","dnk","ft","fg","tp","blk","stl","drb","pss","reb","dovr", "dpot","dovr", "dpot","dhgt", "dstre","dspd","djmp","dendu","dins","ddnk","dft","dfg","dtp","dblk","dstl","ddrb","dpss","dreb",  "skills"],									
		...
		//		console.log(t.state);
                ...
                if (inputs.season === g.season) {
					...
					//	console.log("players[i].miles[0]: "+players[i].miles[0]);
					//	console.log("g.userTid: "+g.userTid);
					//	console.log("players[i].miles[g.userTid]: "+players[i].miles[g.userTid]);
                            ...
                            if (inputs.tid === g.userTid && (g.phase !== g.PHASE.PLAYOFFS || players.length > 13)) {
                ...
                return dao.players.getAll({
					...
					//console.log("players[0].miles[t.tid]: "+players[0].miles[t.tid]);					
					//console.log("players[0].miles[g.userTid]: "+players[0].miles[g.userTid]);					
                    for (i = 0; i < players.length; i++) {
                        players[i].age = players[i].age - (g.season - inputs.season);
//                        players[i].miles = players[i].miles[t.tid];
                        players[i].miles = players[i].miles[g.userTid];
                        players[i].canRelease = false;
                    }
    ...
    function uiFirst(vm) {
        ...
        $("#roster").on("click", "button", function () {
            ...
            if (this.dataset.action === "release") {
                ...
                if (justDrafted) {
                    releaseMessage = "Are you sure you want to release " + players[i].name() + "?  He will become available for other teams to recruit and will no longer take up a roster spot on your team.";
                } else {
                    releaseMessage = "Are you sure you want to release " + players[i].name() + "?  He will become available for other teams to recruit and will no longer take up a roster spot on your team.";
                }
        ...
        $("#help-roster-release").popover({
            ...
            html: true,
            content: "<p>To free up a roster spot, you can release a player from your team."
        });

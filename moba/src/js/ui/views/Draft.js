...
class Draft extends React.Component {
    ...
    async draftUntilUserOrEndFantasy() {
        const pids = await toWorker('draftUntilUserOrEndFantasy');
        this.savePids(pids);
        await realtimeUpdate(["playerMovement"]);
    }

    async draftUserFantasy(pid) {
        await toWorker('draftUserFantasy', pid);
        this.savePids([pid]);
        await realtimeUpdate(["playerMovement"]);
        await this.draftUntilUserOrEndFantasy(); // Needed for when user has #1 pick in fantasy draft, otherwise no
    }
    ...
    render() {
        ...
        const colsUndrafted = getCols('Name', 'Country', 'Languages', 'Region','Pos', 'Age', 'Ovr', 'Pot', 'Draft');
        colsUndrafted[0].width = '100%';

        if (fantasyDraft) {
            colsUndrafted.splice(5, 0, ...getCols('Contract'));
        }
        ...
        const rowsUndrafted = undrafted.map(p => {
            const data = [
                ...
                p.ratings.ovr,
                p.ratings.pot,
                <button className="btn btn-xs btn-primary" disabled={!usersTurn} onClick={() => this.draftUserFantasy(p.pid)}>Draft</button>,
            ];
...
//        const rowsDrafted = drafted.map((p,i) => {
            const data = [
                ...
                p.pid >= 0 ? p.born.country : null,
                p.pid >= 0 ? p.ratings.languagesGrouped : null,
                p.pid >= 0 ? p.ratings.region : null,
            ...
            if (fantasyDraft) {
                data.splice(7, 0,
                    p.pid >= 0 ? `${helpers.formatCurrency(p.contract.amount, 'K')} thru ${p.contract.exp}` : null,
                );
            }
        ...
        return <div>
            ...
            {started ? null : <p><button className="btn btn-large btn-success" onClick={() => this.draftUntilUserOrEndFantasy()}>Start Draft</button></p>}

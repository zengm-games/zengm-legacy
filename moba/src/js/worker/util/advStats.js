...
async function calculatePER() {
    ...
    const teams = await idb.getCopies.teamsPlus({
        ...
        season: g.season,
        playoffs: (g.PHASE.PLAYOFFS === g.phase || g.PHASE.MSI === g.phase),
        regularSeason: (g.PHASE.PLAYOFFS !== g.phase && g.PHASE.MSI !== g.phase),
        statType: 'totals',
    });
   ...
   /* for (let i = 0; i < players.length; i++) {
        ...
        players[i].active = g.PHASE.PLAYOFFS !== g.phase || (g.PHASE.PLAYOFFS === g.phase && !_.isEmpty(players[i].stats));

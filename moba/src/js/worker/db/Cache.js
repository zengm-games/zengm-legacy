...
export type Store = 'awards' | 'champions' | 'championPatch' | 'draftOrder' | 'draftPicks' | 'events' | 'gameAttributes' | 'games' | 'messages' | 'msiSeries' | 'negotiations' | 'playerFeats' | 'playerStats' | 'players' | 'playoffSeries' | 'releasedPlayers' | 'schedule' | 'teamSeasons' | 'teamStats' | 'teams' | 'trade';
...
export const STORES: Store[] = ['awards', 'champions', 'championPatch', 'draftOrder', 'draftPicks', 'events', 'gameAttributes', 'games', 'messages', 'msiSeries', 'negotiations', 'playerFeats', 'playerStats', 'players', 'playoffSeries', 'releasedPlayers', 'schedule', 'teamSeasons', 'teamStats', 'teams', 'trade'];
    ...
    constructor() {
        ...
        this.storeInfos = {
            ...
            champions: {
                ...
                pkType: 'number',
                autoIncrement: false,
                getData: (tx: BackboardTx) => tx.champions.getAll(),
            },
            championPatch: {
                pk: 'cpid',
                pkType: 'number',
...
//console.log(`${g.phase} flush duration: ${entries[entries.length - 1].duration / 1000} seconds`);
    ...
    async _clear(store: Store) {
        ...
        if (['negotiations', 'releasedPlayers', 'schedule', 'teamSeasons'].includes(store)) { // added champions for testing purposes

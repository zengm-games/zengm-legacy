...
const processSeasonAttrs = async (output: TeamFiltered, t: Team, seasonAttrs: TeamSeasonAttr[], season: number | void) => {
    ...
    output.seasonAttrs = await Promise.all(seasons.map(async (ts) => {
        ...
        for (const attr of seasonAttrs) {
            ...
            } else if (attr === 'winpSpring') {
                row.winpSpring = 0;
                if (ts.wonSpring + ts.lostSpring > 0) {
                    row.winpSpring = ts.wonSpring / (ts.wonSpring + ts.lostSpring);
                }
            } else if (attr === 'winpSummer') {
                row.winpSummer = 0;
                if (ts.wonSummer + ts.lostSummer > 0) {
                    row.winpSummer = ts.wonSummer / (ts.wonSummer + ts.lostSummer);
                }
            } else if (attr === 'pointsYear') {
                row.pointsYear = 0;
				if (ts.pointsSummer == "AQ") {
					row.pointsYear += 100000;
				} else if (ts.pointsSummer >= 0) {
					row.pointsYear += ts.pointsSummer;
				} else {
				}
				if (ts.pointsSpring >= 0) {
					row.pointsYear += ts.pointsSpring;
				} 				
				//pointsSpring
				//pointsSummer
             //   if (ts.wonSummer + ts.lostSummer > 0) {
...
const processStats = async (
	...
	/*          } else if (stat === 'fgp') {
                    ...
                    if (statType === 'totals') {
                        row.diff = ts.pf - ts.oppTw;
                    } else {
                        row.diff = (ts.pf - ts.oppTw) / ts.gp;
                    }

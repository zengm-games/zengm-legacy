    ...
    [0, 1].forEach(i => {
        promises.push(idb.cache.players.indexGetAll('playersByTid', tids[i]).then(async (playersTemp) => {
            ...
            players = await idb.getCopies.playersPlus(players, {
                attrs: ['pid', 'name', 'contract','born'],
				ratings: ['ovr','pot','season'],
                season: g.season,
                tid: tids[i],
                showRookies: true,
                showNoStats: true,
...
const formatAssetsEventLog = (t) => {
	...
	//console.log(t);
    t.trade.forEach(p => strings.push(`<a href="${helpers.leagueUrl(["player", p.pid])}">${p.name} </a> (${p.ratings.season - p.born.year}, ${p.ratings.ovr}/${p.ratings.pot}, ${p.born.loc}, ${p.born.country})`));
...
const processTrade = async (tradeSummary, tids, pids, dpids) => {
    ...
    logEvent({
        type: "trade",
        text: `<a href="${helpers.leagueUrl(["roster", g.teamAbbrevsCache[tids[0]], g.season])}">${g.teamNamesCache[tids[0]]}</a> (${g.teamCountryCache[tids[0]]}) traded ${formatAssetsEventLog(tradeSummary.teams[0])} to <a href="${helpers.leagueUrl(["roster", g.teamAbbrevsCache[tids[1]], g.season])}">${g.teamNamesCache[tids[1]]}</a>  (${g.teamCountryCache[tids[1]]}) for ${formatAssetsEventLog(tradeSummary.teams[1])}.`,
...
const betweenAiTeams = async () => {
   ...
   // const playersAll = (await idb.getCopies.players()).filter((p) => !isUntradable(p));

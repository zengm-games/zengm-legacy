...
async function create(pid: number, resigning: boolean, tid: number = g.userTid): Promise<string | void> {
    ...
    if (playersOnRoster.length >= 12 && !resigning) {
    ...
    const playerAmount = freeAgents.amountWithMood(p.contract.amount, p.freeAgentMood[g.userTid])/1000;
...
async function accept(pid: number, amount: number, exp: number): Promise<?string> {
   ...
   /* if (!negotiation.resigning && (payroll + amount > g.salaryCap && amount > g.minContract)) {
    ...
    if (negotiation.resigning) {
        logEvent({
            type: "reSigned",
            text: `<a href="${helpers.leagueUrl(["roster", g.teamAbbrevsCache[g.userTid], g.season])}">${g.teamNamesCache[g.userTid]}</a> re-signed <a href="${helpers.leagueUrl(["player", p.pid])}">${p.firstName} ${p.lastName}</a> for ${helpers.formatCurrency(p.contract.amount / 1000, "K")}/year through ${p.contract.exp}.`,
        ...
        logEvent({
            type: "freeAgent",
            text: `<a href="${helpers.leagueUrl(["roster", g.teamAbbrevsCache[g.userTid], g.season])}">${g.teamNamesCache[g.userTid]}</a> signed <a href="${helpers.leagueUrl(["player", p.pid])}">${p.firstName} ${p.lastName}</a> for ${helpers.formatCurrency(p.contract.amount / 1000, "K")}/year through ${p.contract.exp}.`,
            showNotification: false,
            pids: [p.pid],
            tids: [g.userTid],
        });

...
async function updateTrade(): void | {[key: string]: any} {
   ...
   //attrs = ["pid", "name", "userID","pos", "age", "contract", "injury", "watch", "gamesUntilTradable"];
    ...
    const attrs = ["pid", "name", "userID", "age", "contract", "injury", "watch", "gamesUntilTradable","born"];
    const ratings = ["MMR","ovr", "pot", "skills", "pos", "languages", "languagesGrouped"];
	const stats = ["min", "pts", "trb", "ast", "per","tp","fg","fga","fgp","kda"];	
//    const stats = ["min", "pts", "trb", "ast", "per"];
	...
	//	console.log(userRoster[i]);
		userRoster[i].name = userRoster[i].userID;
    ...
    for (let i = 0; i < otherRoster.length; i++) {
		//console.log(otherRoster[i]);
		otherRoster[i].name = otherRoster[i].userID;		
    ...
    vars.showResigningMsg = g.phase > g.PHASE.PLAYOFFS && g.phase < g.PHASE.FREE_AGENCY;

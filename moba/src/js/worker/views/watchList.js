...
async function updatePlayers(
    ...
    if (updateEvents.includes('clearWatchList') || updateEvents.includes('gameSim') || updateEvents.includes('playerMovement') || inputs.statType !== state.statType || inputs.playoffs !== state.playoffs) {
        ...
        players = await idb.getCopies.playersPlus(players, {
			attrs: ["pid", "name", "age", "injury", "tid", "abbrev", "watch", "contract", "freeAgentMood", "draft","born"],
			ratings: ["MMR","ovr", "pot", "skills","pos"],
			stats: ["gp", "min", "fgp", "tpp", "ftp", "trb", "ast", "tov", "stl", "blk", "pts", "per", "ewa","tp","fg","fga","kda"],			

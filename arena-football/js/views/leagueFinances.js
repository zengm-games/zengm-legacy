    ...
    function uiFirst(vm) {
        ...
        ko.computed(function () {
            ...
            ui.datatableSinglePage($("#league-finances"), 5, _.map(vm.teams(), function (t) {
                ...
                return ['<a href="' + helpers.leagueUrl(["team_finances", t.abbrev]) + '">' + t.region + ' ' + t.name + '</a>', helpers.numberWithCommas(helpers.round(t.att)), helpers.formatCurrency(t.revenue, "K"), helpers.formatCurrency(t.profit, "K"), helpers.formatCurrency(t.cash, "K"), helpers.formatCurrency(payroll, "K")];
            }));
        }).extend({throttle: 1});

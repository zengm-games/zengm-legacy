	...
	function newScheduleDefault() {
		...
		for ( i = 0; i < teams.length; i++) {
			...
			while (homeGames < 1) {
				...
				// looping, why not finding right amount of games?
				//							if ((teams[i].did !== teams[k].did) &&  (teams[k].awayGames <4) && (teams[i].cid !== teams[k].cid) ) {
				if ((teams[k].awayGames < 8) && (teams[i].cid !== teams[k].cid)) {
	...
	function newPhasePlayoffs(cb) {
		...
		team.filter({
			...
			for ( cid = 0; cid < 2; cid++) {
				...
				/*                series[1][0  + cid*2 ] = {home: teamsConf[3], away: teamsConf[4]};
				 series[1][0 + cid*2 ].home.seed = 4;
				 series[1][0 + cid*2 ].away.seed = 5;

				 series[1][1  + cid*2 ] = {home: teamsConf[2], away: teamsConf[5]};
				 series[1][1 + cid*2 ].home.seed = 3;
				 series[1][1 + cid*2 ].away.seed = 6; */

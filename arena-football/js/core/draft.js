...
define(["db", "globals", "ui", "core/finances", "core/player", "core/team", "util/helpers", "util/random"], function(db, g, ui, finances, player, team, helpers, random) {
	...
	function genPlayers(ot, tid, scoutingRank, numPlayers, cb) {
		...
		if (numPlayers === null || numPlayers === undefined) {
			...
			numPlayers = Math.round(185 * g.numTeams / 32);
			// 70 scaled by number of teams
		}
	...
	function getRookieSalaries() {
		...
		rookieSalaries = [50000, 49000, 48000, 47000, 46000, 45000, 44000, 43000, 42000, 41000, 40000, 39000, 38000, 37000, 36000, 35000, 34000, 33000, 31000, 31000, 31000, 31000, 31000, 31000, 31000, 31000, 31000, 31000, 31000, 31000, 31000, 31000, 31000, 31000, 31000, 31000, 31000, 31000, 31000, 31000, 31000, 31000, 31000, 31000, 31000, 31000, 31000];

		//        while (g.numTeams * 4 > rookieSalaries.length) {
		while (g.numTeams * 5 > rookieSalaries.length) {
			// Add min contracts on to end
			rookieSalaries.push(31000);
		}
	...
	function untilUserOrEnd(cb) {
		...
		g.dbl.transaction("players").objectStore("players").index("tid").getAll(g.PLAYER.UNDRAFTED).onsuccess = function(event) {
			...
			playersAll.sort(function(a, b) {
				return player.value(b) - player.value(a);
			});
			...
			getOrder(function(draftOrder) {
				...
				autoSelectPlayer = function() {
					if (draftOrder.length > 0) {
						...
						selection = Math.floor(Math.abs(random.gauss(0, 2)));
						...
						selectPlayer(pick, pid, function() {
							pids.push(pid);
							playersAll.splice(selection, 1);
							// Delete from the list of undrafted players

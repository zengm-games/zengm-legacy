...
//console.log('writeTeamStats 2');
            tx.objectStore("teams").openCursor(that.team[t1].id).onsuccess = function (event) {
                ...
                if (!that.playoffs) {
                    ...
                    scoutingPaid = 7*t.budget.scouting.amount / 16;
                    coachingPaid = 20*t.budget.coaching.amount / 16;
                    healthPaid = 50*t.budget.health.amount / 16;
                    facilitiesPaid = 200*t.budget.facilities.amount / 16;
                    merchRevenue = 100 * att / 1000*82/16*5;
                    if (merchRevenue > 2500*82/16*5) {
                        merchRevenue = 2500*82/16*5;
                    }
                    sponsorRevenue = 100 * att / 1000*82/16*5;
                    if (sponsorRevenue > 6000*82/16*5) {
                        sponsorRevenue = 6000*82/16*5;
                    }
                    nationalTvRevenue = 82/16*15;
                    localTvRevenue = 1 * att / 1000*82/16*5;
                    if (localTvRevenue > 12000*82/16*5) {
                        localTvRevenue = 12000*82/16*5;
                    }
                }
                ...
                att *= 50 / t.budget.ticketPrice.amount ;  // Attendance depends on ticket price. Not sure if this formula is reasonable.
...
//                ticketRevenue = t.budget.ticketPrice.amount * att / 1000;  // [thousands of dollars]
                ticketRevenue = t.budget.ticketPrice.amount * att ;  // [thousands of dollars]
...
// console.log("cbPlayGames3");				
                    cbNoGames();
					
                } else {
// console.log("cbPlayGames4");								

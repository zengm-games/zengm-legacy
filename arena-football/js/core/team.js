	...
	/*	var leftOff;
		leftOff = 0;
			for (j = 0; j < 8; j++) {		
				...
				if ((players[j].pos != "QB") && (j==0)) {
					for (i = j; i < players.length; i++) {
					   if (players[i].pos === "QB") {
		...
		weakestDefender = 15;				
            for (i = 0; i < 8; i++) {
			    if (((players[i].pos != "QB") && (players[i].pos != "OL") && (players[i].pos != "WR")  && (players[i].pos != "RB") && (players[i].pos != "TE") ) && (i<8)) {
...
// sort before defense	
		...
		//	console.log(lastPosition);
            ...
            for (i = 8; i < 16; i++) {			
				if (((players[i].pos != "CB") && (players[i].pos != "DL") && (players[i].pos != "S") && (players[i].pos != "LB") ) && ((i>7) && (i<16))) {
					for (k = (i+1); k < players.length; k++) {
...
/////////////////// kicker			
            ...
            for (i = 16; i < 17; i++) {			
				...
				if (((players[i].pos != "K") && (players[i].pos != "P")) && ((i>15) && (i<17))) {
					for (k = (i+1); k < players.length; k++) {
						//console.log("pos: "+ players[k].pos);
...
//console.log(t.abbrev + " switch to rebuilding")
    ...
    function checkRosterSizes(cb) {
        var checkRosterSize, minFreeAgents,minFreeAgentsQB,minFreeAgentsOL,minFreeAgentsWR,minFreeAgentsK,minFreeAgentsRB,minFreeAgentsDL,minFreeAgentsLB,minFreeAgentsS,minFreeAgentsCB,minFreeAgentsOffense,minFreeAgentsDefense, playerStore, tx, userTeamSizeError;
...
//                        for (i = 0; i < (numPlayersOnRoster - 15); i++) {
//                            if (players[i].pos == "QB") {
							if ( (players[i].pos === "RB" ) || (players[i].pos === "TE" ) ) {
								player.release(tx, players[i], false);
							    i = numPlayersOnRoster;
								numPlayersOnRoster -= 1;	
								hasRB -= 1;								
...
//                            if (players[i].pos == "QB") {
		...
		//			console.log("numPlayers: "+numPlayersOnRoster+" kickers: "+	hasTwoK+" hasDefense: "+hasDefense+" hasCB: " + hasCB+" hasSF: "+hasSF+" hasOffense" + hasOffense+" hasQB: "+hasQB+" hasRB: "+hasRB+" hasTE: "+hasTE);
		//			console.log("numPlayers: "+numPlayersOnRoster+" kickers: "+	hasTwoK+" hasDefense: "+hasDefense+" hasCB: " + hasCB+" hasSF: "+hasSF+" hasOffense" + hasOffense+" hasQB: "+hasQB+" hasRB: "+hasRB+" hasTE: "+hasTE);
				...
				if (numPlayersOnRoster > 20) {
                    if ((tid === g.userTid) && (g.startingSeason != g.season)) {						
						...
						userTeamSizeError = 'Your team currently has more than the maximum number of players (20). You must remove players (by <a href="' + helpers.leagueUrl(["roster"]) + '">releasing them from your roster</a> or through <a href="' + helpers.leagueUrl(["trade"]) + '">trades</a>) before continuing.';
                    } else {
                        // Automatically drop lowest value players until we reach 15
							players.sort(function (a, b) { return player.value(a) - player.value(b); }); // Lowest first
					    i = 0;
                        while (numPlayersOnRoster > 20) {
							player.release(tx, players[i], false);
                            numPlayersOnRoster -= 1;
							i += 1;
                        }
...
//                } else if (numActivePlayersOnRoster != 20 && numActivePlayersOnRoster != 0) {
               } else if (hasTwoK < 1) {
                    if ((tid === g.userTid) && (g.startingSeason != g.season)) {						

                        
                        userTeamSizeError = 'Your team currently need a kicker. You must add a kicker (through <a href="' + helpers.leagueUrl(["free_agents"]) + '">free agency</a> or <a href="' + helpers.leagueUrl(["trade"]) + '">trades</a>) before continuing.';
                    } else {
                    }       
               } else if ((hasDefense+hasCB+hasSF) < 8) {
                    if ((tid === g.userTid) && (g.startingSeason != g.season)) {						
                        
                        userTeamSizeError = 'Your team needs at least 8 players on defense. You must add players (through <a href="' + helpers.leagueUrl(["free_agents"]) + '">free agency</a> or <a href="' + helpers.leagueUrl(["trade"]) + '">trades</a>) before continuing.';
                    } else {
                    }
               } else if ((hasOffense+hasQB+hasRB+hasTE+hasWR) < 8) {
                    if ((tid === g.userTid) && (g.startingSeason != g.season)) {						
                        
                        userTeamSizeError = 'Your team needs at least 8 players on on offense. You must add players (through <a href="' + helpers.leagueUrl(["free_agents"]) + '">free agency</a> or <a href="' + helpers.leagueUrl(["trade"]) + '">trades</a>) before continuing.';
                    } else {
                    }
                }   				
				
/*				else if (numPlayersOnRoster > 20) {
                    if (tid === g.userTid) {
						if (numPlayersOnRoster > 20) {
						//	console.log("too many"+numActivePlayersOnRoster);
							userTeamSizeError = 'Your team currently has more than the maximum number of players (20). You can fix this by trading or releasing players.';
				//		} else {
				//			userTeamSizeError = 'Your team currently has less than the required number of active players (20). You can fix this at the roster page by moving players from inactive to active.';
                        ...
                        // Automatically drop lowest value players until we reach 15
                        players.sort(function (a, b) { return player.value(a) - player.value(b); }); // Lowest first
                        for (i = 0; i < (numPlayersOnRoster - 20); i++) {
//                        for (i = 0; i < (numPlayersOnRoster - 15); i++) {
                            player.release(tx, players[i], false);
                        }
                    }
        ...
        playerStore.index("tid").getAll(g.PLAYER.FREE_AGENT).onsuccess = function (event) {
            ...
            minFreeAgentsQB = [];
            for (i = 0; i < players.length; i++) {
                if ((players[i].contract.amount <= 200000) && (players[i].pos == "QB")) {
            ...
            minFreeAgentsRB = [];
            for (i = 0; i < players.length; i++) {
                if ((players[i].contract.amount <= 200000) && ((players[i].pos == "RB") || (players[i].pos == "TE"))) {
            ...
            minFreeAgentsWR = [];
            for (i = 0; i < players.length; i++) {
                if ((players[i].contract.amount <= 200000) && (players[i].pos == "WR")) {
            ...
            minFreeAgentsK = [];
            for (i = 0; i < players.length; i++) {
                if ((players[i].contract.amount <= 200000) && (players[i].pos == "K")) {
            ...
            minFreeAgentsOL = [];
            for (i = 0; i < players.length; i++) {
                if ((players[i].contract.amount <= 200000) && (players[i].pos == "OL")) {
            ...
            minFreeAgentsDL = [];
            for (i = 0; i < players.length; i++) {
                if ((players[i].contract.amount <= 200000) && (players[i].pos == "DL")) {
                    minFreeAgentsDL.push(players[i]);
                }
            }
            minFreeAgentsDL.sort(function (a, b) { return player.value(b) - player.value(a); });

            minFreeAgentsLB = [];
            for (i = 0; i < players.length; i++) {
                if ((players[i].contract.amount <= 200000) && ((players[i].pos == "LB") || (players[i].pos == "DL"))) {
                    minFreeAgentsLB.push(players[i]);
                }
            }
            minFreeAgentsLB.sort(function (a, b) { return player.value(b) - player.value(a); });

            minFreeAgentsCB = [];
            for (i = 0; i < players.length; i++) {
                if ((players[i].contract.amount <= 200000) && ((players[i].pos == "CB") || (players[i].pos == "S"))) {
                    minFreeAgentsCB.push(players[i]);
                }
            }
            minFreeAgentsCB.sort(function (a, b) { return player.value(b) - player.value(a); });

            minFreeAgentsS = [];
            for (i = 0; i < players.length; i++) {
                if ((players[i].contract.amount <= 200000) && (players[i].pos == "S")) {
                    minFreeAgentsS.push(players[i]);
                }
            }
            minFreeAgentsS.sort(function (a, b) { return player.value(b) - player.value(a); });

			
            minFreeAgentsOffense = [];
            for (i = 0; i < players.length; i++) {
//                if ((players[i].contract.amount <= 200000) && ((players[i].pos == "WR") || (players[i].pos == "OL") ) ) {
                if ((players[i].contract.amount <= 200000) && ((players[i].pos == "OL") ) ) {
                    minFreeAgentsOffense.push(players[i]);
                }
            }
            minFreeAgentsOffense.sort(function (a, b) { return player.value(b) - player.value(a); });


            minFreeAgentsDefense = [];
            for (i = 0; i < players.length; i++) {
                if ((players[i].contract.amount <= 200000) && ((players[i].pos == "DL") || (players[i].pos == "LB")  ) ) {
                    minFreeAgentsDefense.push(players[i]);
                }
            }
            minFreeAgentsDefense.sort(function (a, b) { return player.value(b) - player.value(a); });
...
//                if (players[i].contract.amount <= 400) {
                if ((players[i].contract.amount <= 40000) && (players[i].pos != "QB") && (players[i].pos != "K") && (players[i].pos != "RB") && (players[i].pos != "OL") && (players[i].pos != "WR")) {

            ...
            return Promise.all([
                ...
                history.reverse(); // Show most recent season first
                    ...
                    players = player.filter(players, {
                        ...
                        stats: ["season", "abbrev", "gp", "min", "pts", "trb", "ast", "per", "ewa","qbr","ruya","reyc","fgaMidRange","gp","fgpAtRim","olr"],
                        tid: inputs.tid
                    });
    ...
    function uiFirst(vm) {
        ...
        ko.computed(function () {
            ui.datatable($("#team-history-players"), 2, _.map(vm.players(), function (p) {
                return [helpers.playerNameLabels(p.pid, p.name, p.injury, [], p.watch), p.pos, String(p.careerStats.gp), helpers.round(p.careerStats.qbr, 1), helpers.round(p.careerStats.ruya, 1), helpers.round(p.careerStats.reyc, 1), helpers.round(p.careerStats.olr, 0), helpers.round(p.careerStats.fgpAtRim, 1), helpers.round(p.careerStats.fgaMidRange, 0), p.lastYr, p.hof, p.tid > g.PLAYER.RETIRED && p.tid !== vm.team.tid(), p.tid === vm.team.tid()];
            }), {

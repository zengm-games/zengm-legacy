...
define(["dao", "globals", "ui", "core/player", "lib/bluebird", "lib/underscore", "util/bbgmView", "util/helpers"], function (dao, g, ui, player, Promise, _, bbgmView, helpers) {
    ...
    function genFileName(leagueName, season, grouping) {
        ...
        fileName = "FBGM_" + leagueName.replace(/[^a-z0-9]/gi, '_') + "_" + season + "_" + (season === "all" ? "seasons" : "season") + (grouping === "averages" ? "_Average_Stats" : "_Game_Stats");
    ...
    function playerAveragesCSV(season) {
        return dao.players.getAll({
            ...
            output = "pid,Name,Pos,Age,Team,Season,GP,GS,Min,PassM,PassA,Pass%,PassTY,PassYA,PassTD,PassTD%,PassInt,PassInt%,QBR,RushA,RushTY,RushYA,RushTD,Rec,RecTY,RecYA,RecTD,LB,PA,RPA/PI,TP,FGM,FGA,FG%,Sacks,RA,PA,RPA/PI,TP\n";

            seasons.forEach(function (s) {
                player.filter(players, {
                    attrs: ["pid", "name", "pos", "age"],
                    stats: ["abbrev", "gp", "gs", "min", "fg", "fga", "fgp", "tp", "tpa", "tpp", "ft", "fta", "ftp", "orb", "drb", "trb", "ast", "tov", "stl", "blk", "pf", "pts", "per", "ewa","pya","tdp","inter","intp","tya","sackp","ruya","reyc","qbr", "fgAtRim", "fgaAtRim", "fgpAtRim", "fgLowPost", "fgaLowPost", "fgpLowPost", "fgMidRange", "fgaMidRange", "fgpMidRange", "tp", "tpa", "tpp","olr","olp","olrmpa","der","dep","derpatp","olc","dec"],
                    season: s
                }).forEach(function (p) {
                    output += [p.pid, p.name, p.pos, p.age, p.stats.abbrev, s, p.stats.gp, p.stats.gs, p.stats.min, p.stats.fg, p.stats.fga, p.stats.fgp, p.stats.stl, p.stats.pyu, p.stats.blk, p.stats.tdp, p.stats.inter, p.stats.intp, p.stats.qbr, p.stats.tov, p.stats.drb, p.stats.ruya, p.stats.ft, p.stats.ast, p.stats.orb, p.stats.reyc, p.stats.pf, p.stats.olr, p.stats.olp, p.stats.olrmpa, p.stats.olc, p.stats.fgAtRim, p.stats.fgaAtRim, p.stats.fgpAtRim, p.stats.fgaMidRange, p.stats.der, p.stats.dep, p.stats.derpatp, p.stats.dec].join(",") + "\n";
                });
            });
    ...
    function playerGamesCSV(season) {
        ...
        return gamesPromise.then(function (games) {
            ...
            output = "pid,Name,Pos,Team,Opp,Score,WL,Season,Playoffs,Min,PassM,PassA,Pass%,PassTY,PassYA,PassTD,PassTD%,PassInt,PassInt%,QBR,RushA,RushTY,RushYA,RushTD,Rec,RecTY,RecYA,RecTD,LB,PA,RPA/PI,TP,FGM,FGA,FG%,Sacks,RA,PA,RPA/PI,TP\n";
            ...
            for (i = 0; i < teams.length; i++) {
                for (j = 0; j < 2; j++) {
                    ...
                    t.players.forEach(function (p) {
                        output += [p.pid, p.name, p.pos, g.teamAbbrevsCache[t.tid], g.teamAbbrevsCache[t2.tid], t.pts + "-" + t2.pts, t.pts > t2.pts ? "W" : "L", seasons[i],  games[i].playoffs, p.min, p.fg, p.fga, p.fgp, p.stl, p.pyu, p.blk, p.tdp, p.inter, p.intp, p.qbr, p.tov, p.drb, p.ruya, p.ft, p.ast, p.orb, p.reyc, p.pf, p.olr, p.olp, p.olrmpa, p.olc, p.fgAtRim, p.fgaAtRim, p.fgpAtRim, p.fgaMidRange, p.der, p.dep, p.derpatp, p.dec].join(",") + "\n";
                    });
                }
            }

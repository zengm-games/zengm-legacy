        ...
        if (updateEvents.indexOf("dbChange") >= 0 || (inputs.season === g.season && (updateEvents.indexOf("gameSim") >= 0 || updateEvents.indexOf("playerMovement") >= 0)) || inputs.season !== vm.season()) {
            return dao.players.getAll({
                ...
                players = player.filter(players, {
                    ...
                    ratings: ["skills"],
                    stats: ["abbrev", "gp", "gs", "min", "fgAtRim", "fgaAtRim", "fgpAtRim", "fgLowPost", "fgaLowPost", "fgpLowPost", "fgMidRange", "fgaMidRange", "fgpMidRange", "tp", "tpa", "tpp","olr","olp","olrmpa","der","dep","derpatp","olc","dec","intery"],
...
//                return [helpers.playerNameLabels(p.pid, p.name, p.injury, p.ratings.skills, p.watch), p.pos, '<a href="' + helpers.leagueUrl(["roster", p.stats.abbrev, season]) + '">' + p.stats.abbrev + '</a>', String(p.stats.gp), String(p.stats.gs), helpers.round(p.stats.olr, 0), helpers.round(p.stats.olp, 0), helpers.round(p.stats.olrmpa, 2), helpers.round(p.stats.olc, 0), helpers.round(p.stats.fgAtRim, 0), helpers.round(p.stats.fgaAtRim, 0), helpers.round(p.stats.fgpAtRim, 1), helpers.round(p.stats.fgaMidRange, 0), helpers.round(p.stats.der, 0), helpers.round(p.stats.dep, 0), helpers.round(p.stats.derpatp, 2), helpers.round(p.stats.dec, 0), helpers.round(p.stats.tpa, 1), helpers.round(p.stats.tpp, 1)];
                return [helpers.playerNameLabels(p.pid, p.name, p.injury, p.ratings.skills, p.watch), p.pos, '<a href="' + helpers.leagueUrl(["roster", p.stats.abbrev, season]) + '">' + p.stats.abbrev + '</a>', String(p.stats.gp), String(p.stats.gs), helpers.round(p.stats.olr, 0), helpers.round(p.stats.olp, 0), helpers.round(p.stats.olrmpa, 2), helpers.round(p.stats.olc, 0), helpers.round(p.stats.fgAtRim, 0), helpers.round(p.stats.fgaAtRim, 0), helpers.round(p.stats.fgpAtRim, 1), helpers.round(p.stats.fgaMidRange, 0), helpers.round(p.stats.intery, 0),helpers.round(p.stats.der, 0), helpers.round(p.stats.dep, 0), helpers.round(p.stats.derpatp, 2), helpers.round(p.stats.dec, 0)];
            }));
        }).extend({throttle: 1});

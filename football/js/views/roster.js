...
define(["dao", "globals", "ui", "core/league", "core/player", "core/season", "core/team", "core/trade", "lib/bluebird", "lib/knockout", "lib/jquery", "views/components", "util/bbgmView", "util/helpers"], function (dao, g, ui, league, player, season, team, trade, Promise, ko, $, components, bbgmView, helpers) {
    ...
    function doRelease(pid, justDrafted) {
        ...
        return dao.players.count({
            ...
            if (numPlayersOnRoster <= 45) {
                return "You must keep at least 45 players on your roster.";
            }
...
//            return 0 - this.players.off.active().length;
            return 53 - this.players.off.active().length - this.players.def.active().length - this.players.k.active().length- this.players.off.inactive().length - this.players.def.inactive().length - this.players.k.inactive().length
        }, this);
        this.numActiveSpots = ko.computed(function () {
            var count;
            count = 45; // Total number allowed
...
//            return 0 - this.players.off.active().length;
        ...
        this.numActiveSpots = ko.computed(function () {
            return 45 - this.players.off.active().length - this.players.def.active().length - this.players.k.active().length
    ...
    function updateRoster(inputs, updateEvents, vm) {
        ...
        if (updateEvents.indexOf("dbChange") >= 0 || (inputs.season === g.season && (updateEvents.indexOf("gameSim") >= 0 || updateEvents.indexOf("playerMovement") >= 0)) || inputs.abbrev !== vm.abbrev() || inputs.season !== vm.season()) {
            ...
            return team.filter({
                ...
                stats = ["gp", "min", "pts", "trb", "ast", "per","qbr","ruya","reyc","olrmpa","fgAtRim","fgaAtRim","fgpAtRim","fgLowPost","fgaLowPost","fgpLowPost","punta","puntty","punttb","derpatp","fg","fga","fgp","stl","pya","blk","inter","drb","orb","olr","olp","fgaMidRange","derpa","der","dep","intery","dec", "yearsWithTeam"];
                    ...
                    return Promise.all([
                        ...
                        for (i = 0; i < schedule.length; i++) {
                            ...
                            for (i = 0; i < players.length; i++) {							
								...
								if (inputs.tid === g.userTid && (g.phase !== g.PHASE.PLAYOFFS  || players.length > 53)) {
...
//									players = vm.players.off.inactive();
			...
			//	console.log("yearsWithTeam: "+players[i].yearsWithTeam);
			//	console.log("yearsWithTeam: "+players[i].yearsWithTeam());
			//	console.log("yearsWithTeam: "+players[i].draft.year());
...
/*                if (justDrafted) {
                ...
                } else if (players[i].yearsWithTeam() > 1) {
//                    releaseMessage = "Are you sure you want to release " + players[i].name() + "?  He will become a free agent and no longer take up a roster spot on your team. Because his remaining contract isn't guaranteed you will not have to pay his contract.";
                    releaseMessage = "Are you sure you want to release " + players[i].name() + "?  He will become a free agent and no longer take up a roster spot on your team. Because his contract isn't guaranteed you will not have to pay his contract.";
                } else {
                    releaseMessage = "Are you sure you want to release " + players[i].name() + "?  He will become a free agent and no longer take up a roster spot on your team, but you will still have to pay his salary this year (and have it count against the salary cap) because the first year of contracts are guaranteed (not including resigned players).";
                } */
        ...
        $("#help-roster-release").popover({
            ...
            html: true,
            content: "<p>To free up a roster spot or cap space, you can release a player from your team. You will not have to pay his salary."
        });

...
/*           runningBackRating = (ratings.dnk+ratings.ft+ratings.hgt+ratings.stre+ratings.spd+ratings.jmp)/6*1.1;
		...
		//  ovrRating = (4 * ratings.hgt + ratings.stre + 4 * ratings.spd + 2 * ratings.jmp + 3 * ratings.endu + 3 * ratings.ins + 4 * ratings.dnk + ratings.ft + ratings.fg + 2 * ratings.tp + ratings.blk + ratings.stl + ratings.drb + 3 * ratings.pss + ratings.reb) / 32;
			...
			//safetyRating = ((ratings.reb+ratings.cvr)/2)*1.1/2;
			//tightEndRating = ((ratings.stl+ratings.drb)/2)*1.1/2;
...
//        amount = ((value(p) - 1) / 100 - 0.55) * .75 * (maxAmount - minAmount) + minAmount;
//        amount = ((value(p) - 1) / 100 - 0.50) * 1.20 * (maxAmount - minAmount) + minAmount;
...
Kick "kck"
    ...
    function genRatings(profile, baseRating, pot, season, scoutingRank, tid) {
        ...
        /*if (ratings.blk > 50) {
            ratings.blk = (ratings.blk-50)*.5+50;
        } else {
        }*/
        ratings.blk *=.5;
        ratings.stl *=.5;
...
//            va = gp[i] * (qbr[i] - prls[p.pos]) / 16;
           // if (prls[p.pos] >0) {
//				va = (py[i]+rcy[i]+ruy[i]+inter[i]*2+sacks[i]+olr[i])/(prls[p.pos]);
//				va = (py[i]/900+rcy[i]/270+ruy[i]/230+inter[i]+sacks[i]/3+olr[i]/20);
				va = 0;
				va = (py[i]/1000+rcy[i]/(185+prls[p.pos])+ruy[i]/300+inter[i]/(.90+prls[p.pos]*1)+sacks[i]/(2.85+prls[p.pos])+olr[i]/(12.5+prls[p.pos]*.1));
				//console.log(p.pos+" "+py[i]+" "+rcy[i]+" "+ruy[i]+" "+inter[i]+" "+sacks[i]+" "+olr[i]+" "+va);
		//	} else {
				//va = 0;
		//	}
...
//        if (ewa + df > 60) {
//        if (ewa + df > 3) {
//        if (p.awards.length + ewa > 3 && p.awards.length > 1 ) {
		let bonusAwards = 0;
		if (p.pos == "QB") {
			bonusAwards = 2;
		}

                        ...
                        for (j = 0; j < picks.length; j++) {
                            if (dpids[i].indexOf(picks[j].dpid) >= 0) {
							    if (picks[j].round <3) {
									...
									s.teams[i].picks.push({desc: picks[j].season + " " + (picks[j].round === 3 ? "3rd" : "4th") + " round pick (" + g.teamAbbrevsCache[picks[j].originalTid] + ")"});								
								} else {
...
//            default: [75, 73, 71, 69, 68, 67, 66, 65, 64, 63, 62, 61, 60, 59, 58, 57, 56, 55, 54, 53, 52, 51, 50, 50, 50, 49, 49, 49, 48, 48, 48, 47, 47, 47, 46, 46, 46, 45, 45, 45, 44, 44, 44, 43, 43, 43, 42, 42, 42, 41, 41, 41, 40, 40, 39, 39, 38, 38, 37, 37] // This is basically arbitrary
            default: [75, 73, 71, 69, 68, 67, 66, 65, 64, 63, 62, 61, 60, 59, 58, 57, 56, 55, 54, 53, 52, 51, 50, 50, 50, 49, 49, 49, 48, 48, 48, 47, 47, 47, 46, 46, 46, 45, 45, 45, 44, 44, 44, 43, 43, 43, 42, 42, 42, 41, 41, 41, 40, 40, 39, 39, 38, 38, 37, 37, 36.5,36.1,35.7,35.3,34.9,34.5,34.1,33.7,33.3,32.9,32.5,32.1,31.7,31.3,30.9,30.5,30.1,29.7,29.3,28.9,28.5,28.1,27.7,27.3,26.9,26.5,26.1,25.7,25.3,24.9,24.5,24.1,23.7,23.3,22.9,22.5,22.1,21.7,21.3,20.9,20.5,20.1,19.7,19.3,18.9,18.5,18.1,17.7,17.3,16.9,16.5,16.1,15.7,15.3,14.9,14.5,14.1,13.7,13.3,12.9,12.5,12.1,11.7,11.3,10.9,10.5,10.1,9.7,9.3,8.9,8.5,8.1,7.7,7.3,6.9,6.5,6.1,5.7,5.3,4.9,4.5,4.1,3.7,3.3,2.9,2.5,2.1,1.7,1.3,0.9,0.8,.08,.07,.07,.06,.06,.05,.05,.04,.04] // This is basically arbitrary
			
//			36.5	36.1	35.7	35.3	34.9	34.5	34.1	33.7	33.3	32.9	32.5	32.1	31.7	31.3	30.9	30.5	30.1	29.7	29.3	28.9	28.5	28.1	27.7	27.3	26.9	26.5	26.1	25.7	25.3	24.9	24.5	24.1	23.7	23.3	22.9	22.5	22.1	21.7	21.3	20.9	20.5	20.1	19.7	19.3	18.9	18.5	18.1	17.7	17.3	16.9	16.5	16.1	15.7	15.3	14.9	14.5	14.1	13.7	13.3	12.9	12.5	12.1	11.7	11.3	10.9	10.5	10.1	9.7	9.3	8.9	8.5	8.1	7.7	7.3	6.9	6.5	6.1	5.7	5.3	4.9	4.5	4.1	3.7	3.3	2.9	2.5	2.1	1.7	1.3	0.9
		...
		//	console.log(i);
            promises.push(dao.players.getAll({
                ...
                if (players.length > 0) {
                 ...
                 //   for (i = 0; i < players.length; i++) {
                   //     console.log(i+" "+players[i].value)
                    //}		
					//console.log(key+" "+players[0].draft.year);
                    estValues[players[0].draft.year] = _.pluck(players, "value");
				//	console.log(estValues);					
                } else {
               //   for (i = 0; i < players.length; i++) {
                 //       estValues[players[0].draft.year].value += 4; // +4 is to generally make picks more valued
                  //}					
					//console.log(estValues);		
					///console.log(estValues["default"]);		
					//console.log(estValues.default);		
					//console.log(estValues.default[0]);		
					//console.log(estValues["default"][0]);		
					//console.log(estValues.default.length);		
					//console.log(estValues.default);
		...
		// use average of 1-3 years as 4th year's if missing, that way values are closer than the default
		console.log(estValues);
	/*	
		console.log(g.season + 0);		
		console.log(g.season + 1);		
		console.log(g.season + 2);		
		console.log(g.season + 3);		*/
        return Promise.all(promises).then(function () {
            return estValues;
        });
    }

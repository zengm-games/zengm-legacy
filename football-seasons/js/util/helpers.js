...
define(["globals", "lib/jquery", "lib/knockout", "util/eventLog"], function(g, $, ko, eventLog) {
	...
	function getTeamsDefault() {
		...
		//            {tid: 18, cid: 0, did: 3, region: "Phoenix", name: "Vultures", abbrev: "PHO", pop: 3.4},
		{
			tid : 18,
			cid : 1,
			did : 6,
	...
	function bbgmPing(type) {
		if (g.enableLogging) {
			if (type === "league") {
				_gaq.push(["_trackEvent", "Football-Seasons", "New league", g.lid.toString()]);
			} else if (type === "season") {
				_gaq.push(["_trackEvent", "Football-Seasons", "Completed season", g.season.toString()]);
			}
		}
	}

...
"use strict";
    ...
    //    tx = dao.tx(["players", "playerStats", "playoffSeries", "releasedPlayers", "schedule", "teams"], "readwrite");		
        ...
        return team.filter({
            ...
            var cid, i, seednumber,  series, teamsConf, tidPlayoffs, topseeds ;
...
//								if ( (p.value > 40 && Math.random() > .5) || p.value > 45) { // Should eventually be smarter than a coin flip
								console.log(p);// Should eventually be smarter than a coin flip
								let age = g.season - p.born.year;
								console.log(age);
							//	if (  p.value > 50 || (p.value > 45 && Math.random() > .5) ) { // Should eventually be smarter than a coin flip
								if (  age < 28  || (p.value > 45 && Math.random() > .5) ) { // Should eventually be smarter than a coin flip

...
define(["dao", "globals", "core/player", "lib/bluebird", "lib/underscore", "util/eventLog", "util/helpers", "util/random"], function (dao, g, player, Promise, _, eventLog, helpers, random) {
    ...
    function generate(tm) {
        ...
        t = {
            ...
            budget: {
                ticketPrice: {
                    amount: tm.hasOwnProperty("budget") ? tm.budget.ticketPrice.amount : helpers.round(35 + 50 * (g.numTeams - tm.popRank) / (g.numTeams - 1), 2),
                    rank: tm.hasOwnProperty("budget") ? tm.budget.ticketPrice.rank  :  tm.popRank
                },
                scouting: {
                    amount: tm.hasOwnProperty("budget") ? tm.budget.scouting.amount : helpers.round(1600 + 900 * (g.numTeams - tm.popRank) / (g.numTeams - 1)) * 10,
                    rank: tm.hasOwnProperty("budget") ? tm.budget.scouting.rank  : tm.popRank
                },
                coaching: {
                    amount: tm.hasOwnProperty("budget") ? tm.budget.coaching.amount : helpers.round(1600 + 900 * (g.numTeams - tm.popRank) / (g.numTeams - 1)) * 10,
                    rank: tm.hasOwnProperty("budget") ? tm.budget.coaching.rank  : tm.popRank
                },
                health: {
                    amount: tm.hasOwnProperty("budget") ? tm.budget.health.amount : helpers.round(1600 + 900 * (g.numTeams - tm.popRank) / (g.numTeams - 1)) * 10,
                    rank: tm.hasOwnProperty("budget") ? tm.budget.health.rank  : tm.popRank
                },
                facilities: {
                    amount: tm.hasOwnProperty("budget") ? tm.budget.facilities.amount : helpers.round(1600 + 900 * (g.numTeams - tm.popRank) / (g.numTeams - 1)) * 10,
    ...
    function rosterAutoSort(tx, tid) {
        ...
        return dao.players.getAll({
            ...
            players = player.filter(players, {
                attrs: ["pid", "valueNoPot","pos","active","valueNoPotFuzz"],
...
//					   row[stats[j]] = s[stats[j]] * 9 /  s["fta"];
					   ...
					   if (s.fga==0) {
						...
						ft.ISO = (s.ft*1 + s.orb*2 + s.blk*3) / (s.fga) ;
				...
				//			console.log("i: "+i+" false players[i].active: "+players[i].active); // true/false
					...
					if (numPlayersOnRoster > 40) {
						if (g.userTids.indexOf(tid) >= 0 && g.autoPlaySeasons === 0) {
							...
							userTeamSizeError += 'more than the maximum number of players (40). You must remove players (by <a href="' + helpers.leagueUrl(["roster"]) + '">releasing them from your roster</a> or through <a href="' + helpers.leagueUrl(["trade"]) + '">trades</a>) before continuing.';
...
//						if (tid === g.userTid && g.autoPlaySeasons === 0) {
	//						userTeamSizeError = 'Your team currently has more than the maximum number of players (40). You must remove players (by <a href="' + helpers.leagueUrl(["roster"]) + '">releasing them from your roster</a> or through <a href="' + helpers.leagueUrl(["trade"]) + '">trades</a>) before continuing.';
		...
		console.log("checking roster size release");								
...
//						if (tid === g.userTid && g.autoPlaySeasons === 0) {
	...
	//console.log([tid, minFreeAgents.length, numPlayersOnRoster]);
		...
		console.log("checking roster size add "+ minFreeAgents.length);																
	...
	//               } else if (numActivePlayersOnRoster != 25) {
						if (g.userTids.indexOf(tid) >= 0  && g.autoPlaySeasons === 0) {
							...
							if (numActivePlayersOnRoster > 25) {
								userTeamSizeError += 'more than the required number of active players (25). You can fix this at the roster page by moving players from active to inactive.';
							} else {
								userTeamSizeError += 'less than the required number of active players (25). You can fix this at the roster page by moving players from inactive to active.';

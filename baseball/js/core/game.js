    ...
    function writeTeamStats(tx, results) {
        return Promise.reduce([0, 1], function (att, t1) {
            ...
            return Promise.all([
				...
				if (t1 === 1) { // Base on home team				
//					att = 10000 + (0.1 + 0.9 * Math.pow(teamSeason.hype, 2)) * teamSeason.pop * 1000000 * 0.01;  // Base attendance - between 2% and 0.2% of the region
					att = 10000 + (0.1 + 0.9 * Math.pow(teamSeason.hype, 1)*20 + teamSeason.pop) * 1000000 * 0.00125;  // 1.25k, or 5k to 50k Base attendance - between 2% and 0.2% of the region
					att *= 1.1;  // 1.25k, or 5k to 50k Base attendance - between 2% and 0.2% of the region
                ...
                if (g.phase !== g.PHASE.PLAYOFFS) {
                    ...
                    merchRevenue = 6 * att / 1000;
//                    if (merchRevenue > 250) {
//                        merchRevenue = 250;
                    if (merchRevenue > 250) {
                        merchRevenue = 250;
                    }
                    sponsorRevenue = 20 * att / 1000;
//                    if (sponsorRevenue > 600) {
//                        sponsorRevenue = 600;
                    if (sponsorRevenue > 700) {
                        sponsorRevenue = 700;
                    }
                    nationalTvRevenue = 225;
                    localTvRevenue = 20 * att / 1000;
//                    if (localTvRevenue > 1200) {
//                        localTvRevenue = 1200;
                    if (localTvRevenue > 1400) {
                        localTvRevenue = 1400;
                    }
                }
	...
	//                att *= 1 + 0.075 * (30 - finances.getRankLastThree(t, "expenses", "facilities")) / 29;  // Attendance depends on facilities. Not sure if this formula is reasonable.
				//	att *= 1.6;
					att *= g.numTeams / t.budget.ticketPrice.amount*2;  // Attendance depends on ticket price. Not sure if this formula is reasonable.
...
//		        playedGame = ((results.team[t].player[p].stat.fga > 0) || (results.team[t].player[p].stat.fta > 0) || (results.team[t].player[p].stat.fieldAttempts > 0) || (results.team[t].player[p].energy < 100));
		        playedGame = ((p.stat.fga > 0) || (p.stat.fta > 0) || (p.stat.fieldAttempts > 0) || (p.energy < 100));
...
//						if ( (results.team[t].player[p].stat.fga > 0) || (results.team[t].player[p].stat.fta > 0) || (results.team[t].player[p].stat.fieldAttempts > 0) )  {
						if ( (p.stat.fga > 0) || (p.stat.fta > 0) || (p.stat.fieldAttempts > 0) )  {
							ps.gp += 1; // May be a pitcher resting
						}						
                        ...
                        if ((ps.gp % 1 === 0) || injuredThisGame) {
                            dao.players.get({ot: tx, key: p.id}).then(function (p_) {
									if (p.energy < 70) {
										p_.energy = 71;			
					//				player_.energy = that.team[t].player[p].energy+10;			
									} else if (p.energy < 75) {
										p_.energy = 77;			
									} else if (p.energy < 80) {
										p_.energy = 85;			
					...
					//            player_.energy = that.team[t].player[p].energy;			
										p_.energy = 100;			
									}
							
									// make this a game option
								//	console.log(g.posChangeType);
									if (g.posChangeType == 0) {											
										if (( (p.pos == "SP") ||  (p.pos == "RP")  ||  (p.pos == "CL") ) || (p_.ptModifier>1)) {				
											p_.pos = p.pos;																														
										}											
									} else {
										p_.pos = p.pos;																			
...
//        cursor.update(player_);
                ...
                // Only update player object (values and injuries) every 10 regular season games or on injury
                if ((playerStats.gp % 0 === 0 && g.phase !== g.PHASE.PLAYOFFS) || (injuredThisGame)) {
...
//                    gameStats.teams[t].players[p].energy += 1;				
//                    gameStats.teams[t].players[p].energy = 100;		
              //      console.log("before: "+results.team[t].player[p].energy);		
/*					if (results.team[t].player[p].energy < 80) {
					    results.team[t].player[p].energy += 20;
						gameStats.teams[t].players[p].energy = results.team[t].player[p].energy;		
					} else {
						gameStats.teams[t].players[p].energy = 100;		
					} */
                 //   console.log("after: "+gameStats.teams[t].players[p].energy);		
//               					if (gameStats.teams[t].players[p].energy < 100) {
//					}
           ...
           /*     if ((g.userTid === results.team[0].id || g.userTid === results.team[1].id) && playoffSeries.currentRound < 3) {
                ...
                if (series.away.won === 4 || series.home.won === 4) {
                    ...
                    if (playoffSeries.currentRound === 0) {
                        ...
                        currentRoundText = "championship series";
                    }
...
//                        p = {id: players[i].pid, name: players[i].name, pos: players[i].pos, valueNoPot: player.value(players[i], {noPot: true}), stat: {}, compositeRating: {}, skills: [], energy: {}, injured: players[i].injury.type !== "Healthy", ptModifier: players[i].ptModifier, battingOrder: players[i].battingOrder, offDefK: players[i].offDefK, active: players[i].active,rosterOrder: results.team[t].player[p].rosterOrder};
                        p = {id: players[i].pid, name: players[i].name, pos: players[i].pos, valueNoPot: players[i].valueNoPot, stat: {}, compositeRating: {}, skills: [], energy: {},injury: players[i].injury, injured: players[i].injury.type !== "Healthy", ptModifier: players[i].ptModifier, battingOrder: players[i].battingOrder, offDefK: players[i].offDefK, active: players[i].active};

    ...
    function uiFirst(vm) {
        ...
        ko.computed(function () {
            ui.datatable($("#negotiation-list"), 4, _.map(vm.players(), function (p) {
                ...
                return [helpers.playerNameLabels(p.pid, p.name, p.injury, p.ratings.skills, p.watch), p.pos, String(p.age), String(p.ratings.ovr), String(p.ratings.pot), helpers.round(p.stats.fga, 0), helpers.round(p.stats.trb, 3), helpers.round(p.stats.pts, 0), helpers.round(p.stats.blk, 0), helpers.round(p.stats.fta, 0), helpers.round(p.stats.pf, 2), helpers.round(p.stats.fgAtRim, 2), helpers.round(p.stats.war, 2), helpers.formatCurrency(p.contract.amount, "M") + ' thru ' + p.contract.exp, '<div title="' + p.mood.text + '" style="width: 100%; height: 21px; background-color: ' + p.mood.color + '"><span style="display: none">' + p.freeAgentMood[g.userTid] + '</span></div>', negotiateButton];
            }));
        }).extend({throttle: 1});

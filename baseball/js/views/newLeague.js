...
define(["dao", "ui", "core/league", "lib/bluebird", "lib/jquery", "lib/knockout", "lib/knockout.mapping", "util/bbgmView", "util/helpers", "util/viewHelpers"], function (dao, ui, league, Promise, $, ko, komapping, bbgmView, helpers, viewHelpers) {
    ...
    function post(req) {
        Promise.try(function () {
            var file, startingSeason, tid, posid;
        ...
        startingSeason = 2019;
            ...
            tid = parseInt(req.params.tid, 10);
 			posid = Math.floor(req.params.posid);
                ...
                if (file !== undefined) {
                    return new Promise(function (resolve, reject) {
                        ...
                        reader.onload = function (event) {
                            ...
                            league.create(req.params.name, tid,posid, leagueFile, startingSeason, randomizeRosters).then(resolve);
                        };
                    });
                }
            }

            return league.create(req.params.name, tid,posid, null, startingSeason, false);
        }).then(function (lid) {
     ...
     function updateNewLeague() {
        ...
        return dao.leagues.iterate({
            ...
            var teams, posChangeType;

            if (newLid === null) {
                newLid = 1;
            }

			posChangeType = helpers.getPosChangeType();

...
//        if (updateEvents.indexOf("dbChange") >= 0 || updateEvents.indexOf("watchList") >= 0 || updateEvents.indexOf("gameSim") >= 0 || updateEvents.indexOf("playerMovement") >= 0 || inputs.statType !== vm.statType() || inputs.playoffs !== vm.playoffs()) {
            ...
            g.dbl.transaction("players").objectStore("players").openCursor().onsuccess = function (event) {
                ...
                if (cursor) {
                    ...
                    players = player.filter(playersUnfiltered, {
                        ...
                        stats: ["gp", "min", "fgp", "tpp", "ftp", "trb", "ast", "tov", "stl", "blk", "pts", "per", "ewa","save","winP","lossP","war","pf","drb","save","fta","tp"],
                        season: g.season,
        ...
        ko.computed(function () {
            ...
            for (i = 0; i < vm.players().length; i++) {
                ...
                if (vm.playoffs() === "playoffs") {
                    ...
                    ["gp", "min", "fgp", "tpp", "ftp", "trb", "ast", "tov", "stl", "blk", "pts", "per", "ewa","save","winP","lossP","war","pf","drb","save","fta","tp"].forEach(function (category) {
                ...
                rows.push([helpers.playerNameLabels(p.pid, p.name, p.injury, p.ratings.skills, p.watch), p.pos, String(p.age), '<a href="' + helpers.leagueUrl(["roster", p.abbrev]) + '">' + p.abbrev + '</a>', String(p.ratings.ovr), String(p.ratings.pot), contract, helpers.round(p.stats.trb, 3), helpers.round(p.stats.blk, 0), helpers.round(p.stats.pts, 0), helpers.round(p.stats.stl, 0), helpers.round(p.stats.tp, 0), helpers.round(p.stats.drb, 3), helpers.round(p.stats.ftp, 3), helpers.round(p.stats.tpp, 3), helpers.round(p.stats.winP, 0), helpers.round(p.stats.lossP, 0), helpers.round(p.stats.save, 0), helpers.round(p.stats.fta, 0), helpers.round(p.stats.pf, 2), helpers.round(p.stats.war, 2)]);
            }

            ui.datatable($("#watch-list2"), 0, rows);
        }).extend({throttle: 1});

        ui.tableClickableRows($("#watch-list2"));

        document.getElementById("clear-watch-list2").addEventListener("click", function () {
    ...
    function uiEvery(updateEvents, vm) {
        components.dropdown("watch-list2-dropdown", ["statTypes", "playoffs"], [vm.statType(), vm.playoffs()], updateEvents);
    }

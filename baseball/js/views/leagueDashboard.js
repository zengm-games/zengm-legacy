...
define(["dao", "globals", "ui", "core/player", "core/season", "core/team", "lib/knockout", "lib/knockout.mapping", "lib/underscore", "util/bbgmView", "util/helpers"], function (dao, g, ui, player, season, team, ko, komapping, _, bbgmView, helpers) {
    ...
    function updateTeams(inputs, updateEvents) {
        ...
        if (updateEvents.indexOf("dbChange") >= 0 || updateEvents.indexOf("firstRun") >= 0 || updateEvents.indexOf("gameSim") >= 0 || updateEvents.indexOf("playerMovement") >= 0 || updateEvents.indexOf("newPhase") >= 0) {
            ...
            stats = ["pts", "oppPts", "trb", "ast","blk","fgAtRim","pf","trb","wOBA","fgaMidRange","war"];  // This is also used later to find ranks for these team stats
            return team.filter({
                attrs: ["tid", "cid"],
    ...
    function updatePlayers(inputs, updateEvents) {
        ...
        if (updateEvents.indexOf("dbChange") >= 0 || updateEvents.indexOf("firstRun") >= 0 || updateEvents.indexOf("gameSim") >= 0 || updateEvents.indexOf("playerMovement") >= 0 || updateEvents.indexOf("newPhase") >= 0) {
            ...
            return dao.players.getAll({
                ...
                stats = ["pts", "trb", "ast","blk","stl","pf","trb","wOBA","fgAtRim","fgaMidRange","war","fta","winP","lossP","save","fga","tp","ast","tov","fgaAtRim", "yearsWithTeam"];  // This is also used later to find team/league leaders for these player stats

                players = player.filter(players, {
                    attrs: ["pid", "name", "abbrev", "tid", "age", "contract", "rosterOrder", "injury", "watch", "pos","energy","offDefK","active"],
...
//                vars.starters = userPlayers.sort(function (a, b) { return a.rosterOrder - b.rosterOrder; }).slice(0, 5);
                vars.lineup = userPlayers.sort(function (a, b) { return a.rosterOrder - b.rosterOrder; }).slice(0, 8);
                vars.starters = userPlayers.sort(function (a, b) { return a.rosterOrder - b.rosterOrder; }).slice(8, 13);
                vars.closer = userPlayers.sort(function (a, b) { return a.rosterOrder - b.rosterOrder; }).slice(13, 14);
...
/*               vars.lineup = userPlayers.sort(function (a, b) { return a.rosterOrder - b.rosterOrder; }).slice(0, 11);
          ...
          /*      console.log("before");
			...
			//	vars.starters = {};
				...
				//		console.log(" i: "+i+" userPlayers[i].offDefK: "+userPlayers[i].offDefK +" userPlayers[i].active: "+userPlayers[i].active +" userPlayers[i].pos: "+userPlayers[i].pos);
					...
					//		console.log("i: "+i+" offenseCount: "+offenseCount);
						} else if (userPlayers[i].active && (userPlayers[i].offDefK=="def") && (defenseCount < 5)) {

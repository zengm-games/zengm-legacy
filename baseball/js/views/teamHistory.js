    ...
    function updateTeamHistory(inputs, updateEvents, vm) {
        if (updateEvents.indexOf("dbChange") >= 0 || updateEvents.indexOf("firstRun") >= 0 || updateEvents.indexOf("gameSim") >= 0 || inputs.abbrev !== vm.abbrev()) {
            return Promise.all([
                ...
                var   championships, history, i, j, playoffAppearances, totalWon, totalLost, userTeam, userTeamSeason;
                ...
                history.reverse(); // Show most recent season first
                    ...
                    players = player.filter(players, {
                        ...
                        stats: ["season", "abbrev", "gp", "min", "pts", "trb", "ast", "per", "ewa","fga","fta","blk","pf","winP","lossP","save","war","stl"],
                        tid: inputs.tid
                    });
    ...
    function uiFirst(vm) {
        ...
        ko.computed(function () {
            ui.datatable($("#team-history-players"), 2, _.map(vm.players(), function (p) {
                return [helpers.playerNameLabels(p.pid, p.name, p.injury, [], p.watch), p.pos, String(p.careerStats.fga), helpers.round(p.careerStats.trb, 3), helpers.round(p.careerStats.blk, 0), helpers.round(p.careerStats.stl, 0), helpers.round(p.careerStats.fta, 0), helpers.round(p.careerStats.winP, 0), helpers.round(p.careerStats.lossP, 0), helpers.round(p.careerStats.save, 0), helpers.round(p.careerStats.pf, 2), helpers.round(p.careerStats.war, 2), p.lastYr, p.hof, p.tid > g.PLAYER.RETIRED && p.tid !== vm.team.tid(), p.tid === vm.team.tid()];
            }), {

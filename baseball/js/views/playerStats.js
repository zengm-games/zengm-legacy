...
define(["dao", "globals", "ui", "core/player", "lib/jquery", "lib/knockout", "views/components", "util/bbgmView", "util/helpers"], function (dao, g, ui, player, $, ko, components, bbgmView, helpers) {
    ...
    function get(req) {
		...
		// not sure why this is switched around		
		req.params.abbrev = req.params.season;
		req.params.season = req.params.statType;
		req.params.statType = req.params.abbrev;
...
//                    if (min > gp * 5) {
	...
	////		console.log(totalAtBats);
		...
		//
                    if ( (min > (totalAtBats)) && (p.stats.gs > 0)  && (p.stats.fga > 10) && (p.pos != "SP") && (p.pos != "RP")) {					    
                        return true;
                    }
                });
...
//        components.dropdown("player-stats-dropdown", ["seasonsAndCareer", "statTypes", "playoffs"], [vm.season(), vm.statType(), vm.playoffs()], updateEvents);
        components.dropdown("player-stats-dropdown", ["teamsAndAll","seasonsAndCareer", "playoffs"], [vm.abbrev(), vm.season(), vm.playoffs()], updateEvents);
    }

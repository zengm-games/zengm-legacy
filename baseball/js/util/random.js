...
define([], function () {
     ...
     * @param {array} list List to be shuffled in place.
     */
    function shuffleWeek(list,parts) {
        var i,ii, j, l, t, k, partSize,starting;

		//split into parts, only switch parts, not individual days
		// 824 season
		l = list.length;		
//		partSize = (Math.round(list.length/parts/2))+1;
		partSize = (Math.round(list.length/parts/2));
		//if (partSize < 5) {
	//		partSize = 5;
	//	}
		//starting = Math.round(list.length/2);
		for (ii = 0; ii < parts; ii += 2) {		
		//	console.log("NEW PART "+ii+" "+parts+" "+Math.round(list.length/2)+" "+partSize);
			for (i = 1; i < partSize; i++) {
				k = partSize*ii+i;
				j = Math.round(list.length/2)+partSize*ii+i;

				if (j < list.length) {
				//	console.log(j+" "+k)					
					t = list[k];  // swap list[i] and list[j]
					list[k] = list[j];
					list[j] = t;					
				}

...
define(["dao", "globals", "lib/knockout", "util/eventLog"], function (dao, g, ko, eventLog) {
    ...
    function getPosChangeType() {
        ...
        posChangeType = [
            ...
            {posid: 0, name: "Positions Fixed (not including pitchers)"},
            {posid: 1, name: "Players Keep Manager Position Changes From Most Recent Game"}
        ];

      //  teams = addPopRank(teams);

        return posChangeType;
    }		
    ...
    function bbgmPing(type) {
        if (g.enableLogging) {
            if (type === "league") {
                _gaq.push(["_trackEvent", "Baseball", "New league", g.lid.toString()]);
            } else if (type === "season" && g.autoPlaySeasons === 0) {
                _gaq.push(["_trackEvent", "Baseball", "Completed season", g.season.toString()]);
            }
        }
    }
...
//// Basketball
     ...
     */
    function skillsBlockbasketball(skills) {
   ...
   //         Di: "Interior Defender",
    //        Dp: "Perimeter Defender",
     ...
     //       R: "Rebounder",
            ...
            Cl: "Closer",
            St: "Stealer",
            Do: "Defensive Outfielder",
            Di: "Defensive Infielder",
            Pp: "Power Pitcher",
            Fp: "Finesse Pitcher",
            L: "Leader"			
        };
     ...
     * @return {string} Rounded number.
     */
    function roundVaries(value, precision) {
	
	var valueRound3,valueRound0,valueDiff;
	
	    valueRound3 = round(value, 3)
	    valueRound0 = round(value, 0)
		valueDiff = valueRound3-valueRound0;
		if ((valueDiff == 0) ) {
			precision = precision !== undefined ? parseInt(precision, 10) : 0;
		} else {
			precision = 3;
...
/*	    if ((value == 0) || ) {
     ...
     * @return {string} Rounded number.
     */
 /*   function division9(value1,value2) {
         var valuecomb;
         valuecomb = value1/value2*9.00;		 
		  
        return parseFloat(valuecomb).toFixed(2);
    }*/
...
/*                    gSend = JSON.parse(JSON.stringify(g)); // deepCopy fails for some reason
    ...
    return {
       ...
       // division9: division9,		
        ...
        gb: gb,
        checkNaNs: checkNaNs,
		getPosChangeType: getPosChangeType,
